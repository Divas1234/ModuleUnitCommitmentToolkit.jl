...
function add_ramp_constraints!(scuc::Model, NT, NG, NS, units, onoffinit)
    ...
    units_downramp_constr = @constraint(scuc, [s = 1:NS, t = 1:NT],
        ((t == 1) ? units.p_0[:, 1] : pg₀[(1 + (s - 1) * NG):(s * NG), t - 1]) - pg₀[(1 + (s - 1) * NG):(s * NG), t] .<= ramp_down[:, 1] .* x[:, t] + shut_down[:, 1] .* v[:, t] + p_max[:, 1] .* (x[:, t]))
...
function add_pwl_constraints!(scuc::Model, NT, NG, NS, units)
    ...
    units_pwlpower_sum_constr = @constraint(scuc, [s = 1:NS, t = 1:NT, i = 1:NG], pg₀[i + (s - 1) * NG, t] .== p_min[i, 1] * x[i, t] + sum(pgₖ[i + (s - 1) * NG, t, k] for k ∈ 1:num_segments))
...
function add_hydros_constraints!(scuc::Model, NT, NH, NS, hydros)
    ...
    hydros_minpower_constr = @constraint(scuc, [s = 1:NS, t = 1:NT], ph[((s - 1) * NH + 1):(s * NH), t] .>= p_min[:, 1])
    hydros_maxpower_constr = @constraint(scuc, [s = 1:NS, t = 1:NT], ph[((s - 1) * NH + 1):(s * NH), t] .<= p_max[:, 1])
    hydros_resvoir_constr = @constraint(scuc, [s = 1:NS, t = 1:NT], ph[((s - 1) * NH + 1):(s * NH), t] .<= cumsum_resvoir[t, 1])

    hydros_cumsum_reservoir_constr = @constraint(scuc, [s = 1:NS], q0[:, 1] .+ sum(ones(NH, 1) * cumsum_resvoir[t, 1] .- ph[((s - 1) * NH + 1):(s * NH), t] for t ∈ 1:NT) .<= qmax[:, 1])
