...
```
"""
function set_objective_lowcarbon!(scuc::Model, NT, NG, ND, NW, NS, units, config_param, scenarios_prob, refcost::AbstractVector,
        eachslope::AbstractMatrix, emission_factors::AbstractVector, carbon_price::Float64)
    ...
    startup_shutdown = sum(su₀[i, t] + sd₀[i, t] for i ∈ 1:NG, t ∈ 1:NT)

    piecewise_sum = K > 0 ? sum(pgₖ[i + (s - 1) * NG, t, k] * eachslope[k, i] for s ∈ 1:NS, i ∈ 1:NG, t ∈ 1:NT, k ∈ 1:K) : 0.0
    fuel_piecewise = c₀ * (sum(refcost[i] * x[i, t] for i ∈ 1:NG, t ∈ 1:NT) +
                      pₛ * piecewise_sum +
                      pₛ * sum(ρ⁺ * sr⁺[i + (s - 1) * NG, t] + ρ⁻ * sr⁻[i + (s - 1) * NG, t] for s ∈ 1:NS, i ∈ 1:NG, t ∈ 1:NT))

    load_penalty = pₛ * load_curtailment_penalty * sum(Δpd[d + (s - 1) * ND, t] for s ∈ 1:NS, d ∈ 1:ND, t ∈ 1:NT)

    wind_penalty = (NW > 0) ? pₛ * wind_curtailment_penalty * sum(Δpw[w + (s - 1) * NW, t] for s ∈ 1:NS, w ∈ 1:NW, t ∈ 1:NT) : 0.0

    # Carbon emission cost
    carbon_cost = (K > 0) ?
                  pₛ * carbon_price * sum(emission_factors[i] * sum(pgₖ[i + (s - 1) * NG, t, k] for k ∈ 1:K) for s ∈ 1:NS, i ∈ 1:NG, t ∈ 1:NT) : 0.0

    @objective(scuc, Min, startup_shutdown + fuel_piecewise + load_penalty + wind_penalty + carbon_cost)
...
function set_objective_economic!(
        scuc::Model, NT, NG, ND, NW, NS, units, config_param, scenarios_prob, refcost::AbstractVector, eachslope::AbstractMatrix)
    c₀ = config_param.is_CoalPrice
    pₛ = scenarios_prob
    ρ⁺ = 2c₀
    ρ⁻ = 2c₀
    ...
    # Reference (min output) cost: first-stage only
        sum(refcost[i] * x[i, t] for i ∈ 1:NG, t ∈ 1:NT) +
