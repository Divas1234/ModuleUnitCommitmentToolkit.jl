...
function require_data_center_variables!(model::Model)
	missing = [name for name in DATA_CENTER_RESPONSE_VARIABLES if !haskey(JuMP.object_dictionary(model), name)]
	if !isempty(missing)
...
function data_center_variable_refs(model::Model)
	return (
		power = model[:dc_p],
		fv2 = model[:dc_fv²],
		fv2lambda = model[:dc_fv²λ],
		fv2_plus = model[:dc_fv²_plus],
		fv2_minus = model[:dc_fv²_minus],
		fv2_square_plus = model[:dc_fv²_2_plus],
		fv2_square_minus = model[:dc_fv²_2_minus],
		fv2lambda_plus = model[:dc_fv²λ_plus],
		fv2lambda_minus = model[:dc_fv²λ_minus],
		fv2lambda_square_plus = model[:dc_fv²λ_2_plus],
		fv2lambda_square_minus = model[:dc_fv²λ_2_minus],
		weight_fv2_plus = model[:weight_fv²_plus],
		weight_fv2_minus = model[:weight_fv²_minus],
		weight_fv2lambda_plus = model[:weight_fv²λ_plus],
		weight_fv2lambda_minus = model[:weight_fv²λ_minus],
	)
end

function add_data_center_power_constraints!(model::Model, variables, DataCentras, workload::AbstractMatrix, scenario_count::Int, time_count::Int, data_center_count::Int)
	...
	@constraint(model, [s = 1:scenario_count, t = 1:time_count, dc = 1:data_center_count],
		...
		DataCentras.sv_constant[dc] / DataCentras.μ[dc] * workload[dc, t] *
...
function add_data_center_grid_bound_constraints!(model::Model, variables, response_grid, scenario_count::Int, time_count::Int, data_center_count::Int)
	...
	@constraint(model, [s = 1:scenario_count, t = 1:time_count, dc = 1:data_center_count],
		response_grid.fv2lambda_minus_lb[dc] <= variables.fv2lambda_minus[(s - 1) * data_center_count + dc, t] <= response_grid.fv2lambda_minus_ub[dc]
	)
	return nothing
end
...
function add_square_difference_reconstruction_constraints!(model::Model, variables, scenario_count::Int, time_count::Int, data_center_count::Int)
	@constraint(model, [s = 1:scenario_count, t = 1:time_count, dc = 1:data_center_count],
		...
		variables.fv2_square_plus[(s - 1) * data_center_count + dc, t] -
		variables.fv2_square_minus[(s - 1) * data_center_count + dc, t]
	)
	@constraint(model, [s = 1:scenario_count, t = 1:time_count, dc = 1:data_center_count],
		variables.fv2lambda[(s - 1) * data_center_count + dc, t] ==
		variables.fv2lambda_square_plus[(s - 1) * data_center_count + dc, t] -
		variables.fv2lambda_square_minus[(s - 1) * data_center_count + dc, t]
	)
	return nothing
end

function add_convex_grid_reconstruction_constraints!(model::Model, variables, response_grid, scenario_count::Int, time_count::Int, data_center_count::Int, num_breakpoints::Int)
	function add_grid_pair!(value_var, square_var, grid_points, square_grid_points, weight_var)
		@constraint(model, [s = 1:scenario_count, t = 1:time_count, dc = 1:data_center_count],
			...
			sum(grid_points[k] * weight_var[(s - 1) * data_center_count + dc, t, k] for k in 1:num_breakpoints)
		)
		@constraint(model, [s = 1:scenario_count, t = 1:time_count, dc = 1:data_center_count],
			square_var[(s - 1) * data_center_count + dc, t] ==
			sum(square_grid_points[k] * weight_var[(s - 1) * data_center_count + dc, t, k] for k in 1:num_breakpoints)
		)
		@constraint(model, [s = 1:scenario_count, t = 1:time_count, dc = 1:data_center_count],
			sum(weight_var[(s - 1) * data_center_count + dc, t, k] for k in 1:num_breakpoints) == 1
		)
	end

	add_grid_pair!(variables.fv2_plus, variables.fv2_square_plus, response_grid.fv2_plus_grid, response_grid.fv2_2_plus_grid, variables.weight_fv2_plus)
	add_grid_pair!(variables.fv2_minus, variables.fv2_square_minus, response_grid.fv2_minus_grid, response_grid.fv2_2_minus_grid, variables.weight_fv2_minus)
	add_grid_pair!(variables.fv2lambda_plus, variables.fv2lambda_square_plus, response_grid.fv2lambda_plus_grid, response_grid.fv2lambda_2_plus_grid, variables.weight_fv2lambda_plus)
	add_grid_pair!(variables.fv2lambda_minus, variables.fv2lambda_square_minus, response_grid.fv2lambda_minus_grid, response_grid.fv2lambda_2_minus_grid, variables.weight_fv2lambda_minus)
	return nothing
end
...
function add_data_center_response_band_constraints!(model::Model, variables, scenario_count::Int, time_count::Int, data_center_count::Int)
	@constraint(model, [s = 1:scenario_count, t = 1:time_count, dc = 1:data_center_count],
		variables.fv2[(s - 1) * data_center_count + dc, t] <= 1.5
	)
	@constraint(model, [s = 1:scenario_count, t = 1:time_count, dc = 1:data_center_count],
	...
	@constraint(model, [s = 1:scenario_count, t = 1:time_count, dc = 1:data_center_count],
		variables.fv2lambda[(s - 1) * data_center_count + dc, t] <=
		1.05 * variables.fv2[(s - 1) * data_center_count + dc, t]
	)

	for block in data_center_time_blocks(time_count)
		@constraint(model, [s = 1:scenario_count],
			sum(variables.fv2lambda[(s - 1) * data_center_count + dc, t] for dc in 1:data_center_count, t in block.first:block.last) <=
			1.05 * sum(variables.fv2[(s - 1) * data_center_count + dc, t] for dc in 1:data_center_count, t in block.first:block.last)
		)
		@constraint(model, [s = 1:scenario_count],
			sum(variables.fv2lambda[(s - 1) * data_center_count + dc, t] for dc in 1:data_center_count, t in block.first:block.last) >=
			0.95 * sum(variables.fv2[(s - 1) * data_center_count + dc, t] for dc in 1:data_center_count, t in block.first:block.last)
		)
	end
	return nothing
end
