...
function detect_ramp_events_and_overlap(loads::load, winds::wind, start_time::Int64, exec_NT::Int64, max_lookahead::Int64,
    ...
    if !isempty(event_indices)
        ...
        # 仅为足够接近当前执行边界的事件预留交叠。过远事件应由后续滚动窗处理，
        # 否则会无谓扩大当前模型。默认门槛 6h，可按系统响应速度配置。
        event_gate_hours = clamp(parse(Int, get(ENV, "PCM_RAMP_EVENT_GATE_HOURS", "6")), 0, max_lookahead)
        if first_event_idx > event_gate_hours
            adaptive_overlap_verbose() && println(
                "[AdaptiveOverlap][RampCriterion] start=$(start_time) first_event=$(first_event_idx) exceeds gate=$(event_gate_hours)h; T_ramp=0")
            return false, 0
        end
