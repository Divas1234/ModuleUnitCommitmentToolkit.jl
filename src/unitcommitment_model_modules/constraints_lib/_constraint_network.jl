...
function add_transmission_constraints!(
	...
	if config_param.is_NetWorkCon == 1 && Gsdf !== nothing && NL > 0
		...
		if NL == 0 || Gsdf === nothing
			println(
				"\t constraints: 10) transmissionline limits skipped (NL=0 or Gsdf missing)",
			)
			return nothing
		end
		...
		if config_param.is_ConsiderDataCentra == 0
			...
			for l ∈ 1:NL
				...
				push!(
					...
					@constraint(scuc,
						...
						(
							(isnothing(ph) && tem_NH == 0) ? 0 :
							sum(subGsdf_hydros[h] * ph[h, t] for h ∈ 1:tem_NH)
						) +
						sum(
							subGsdf_winds[w] * (
								winds.scenarios_curve[s, t] * winds.p_max[w] -
								Δpw[(s - 1) * NW + w, t]
							) for w ∈ 1:NW
						) - sum(
							subGsdf_loads[d] *
							(loads.load_curve[d, t] - Δpd[(s - 1) * ND + d, t]) for d ∈ 1:ND
						) + sum(
						# Check if NC > 0, storage variables exist, and subGsdf_psses is valid before summing storage contribution
							(
								...
								pc⁺ !== nothing &&
								pc⁻ !== nothing &&
								c <= length(subGsdf_psses) ?
								subGsdf_psses[c] *
								(pc⁻[(s - 1) * NC + c, t] - pc⁺[(s - 1) * NC + c, t]) : 0.0
							) for c ∈ 1:NC
						) <= lines.p_max[l, 1])
				)
				push!(
			...
			# NC > 0 branch - data center constraints
			# Skip DC constraints if DataCentras.locatebus is empty
			if isempty(DataCentras.locatebus) || ND2 == 0
				println("\t constraints: 12) data centra constraints skipped (no data centers)")
				return scuc, [], []
			end

			if !check_var_exists(scuc, "dc_p")
				println("\t constraints: 12) data centra constraints skipped (variables not defined)")
				return scuc, [], []
			end
			dc_p = scuc[:dc_p]
			ph = check_var_exists(scuc, "ph") ? scuc[:ph] : nothing
			...
			for l ∈ 1:NL
				...
				subGsdf_winds = Gsdf[l, winds.index]
				subGsdf_loads = Gsdf[l, loads.locatebus]
				# Handle storage locatebus
				if NC > 0 && !isempty(stroges.locatebus)
					subGsdf_psses = Gsdf[l, stroges.locatebus]
				else
					subGsdf_psses = []
				end

				# Handle data center
				if !isempty(DataCentras.locatebus)
					subGsdf_dc = Gsdf[l, DataCentras.locatebus]
				else
					subGsdf_dc = []
				end

				# Build the constraint expression
				line_power_expr = @expression(scuc,
					[s = 1:NS, t = 1:NT],
					sum(subGsdf_units[i] * pg₀[i + (s - 1) * NG, t] for i ∈ 1:NG) +
					(
						(isnothing(ph) && tem_NH == 0) ? 0 :
					...
					sum(
						...
						subGsdf_loads[d] * (loads.load_curve[d, t] - Δpd[(s - 1) * ND + d, t]) for d ∈ 1:ND
					))

				# Add storage contribution if applicable
				if NC > 0 && pc⁺ !== nothing && pc⁻ !== nothing && !isempty(subGsdf_psses)
					line_power_expr = @expression(scuc,
						[s = 1:NS, t = 1:NT],
						line_power_expr[s, t] +
						sum(subGsdf_psses[c] * (pc⁻[(s - 1) * NC + c, t] - pc⁺[(s - 1) * NC + c, t]) for c ∈ 1:min(NC, length(subGsdf_psses))))
				end

				# Add data center contribution if applicable
				if !isempty(subGsdf_dc)
					line_power_expr = @expression(scuc,
						[s = 1:NS, t = 1:NT],
						line_power_expr[s, t] -
						sum(subGsdf_dc[c] * dc_p[c + (s - 1) * ND2, t] for c ∈ 1:min(ND2, length(subGsdf_dc))))
				end

				push!(transmissionline_powerflow_upbound_constr,
					@constraint(scuc, [s = 1:NS, t = 1:NT], line_power_expr[s, t] <= lines.p_max[l])
				)
				push!(transmissionline_powerflow_downbound_constr,
					@constraint(scuc, [s = 1:NS, t = 1:NT], line_power_expr[s, t] >= lines.p_min[l])
				)
			end
		end
		println("\t constraints: 10) transmissionline limits for baseline\t\t\t done")
		...
		return println(
			"\t constraints: 10) transmissionline limits skipped (is_NetWorkCon != 1 or Gsdf missing or NL=0)",
		)
	end
end
