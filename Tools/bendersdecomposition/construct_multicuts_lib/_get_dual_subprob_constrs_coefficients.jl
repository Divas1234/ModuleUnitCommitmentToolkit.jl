...
function get_dual_constrs_coefficient(
    ...
    for (key, value) in constrs
        ...
        x_coeff, x_sort_order, x_alignment_flag =
            get_x_coeff_vectors_from_constr(key, current_model.model, value, NT, NG)
        u_coeff, u_sort_order, u_alignment_flag =
            get_u_coeff_vectors_from_constr(key, current_model.model, value, NT, NG)
        v_coeff, v_sort_order, v_alignment_flag =
            get_v_coeff_vectors_from_constr(key, current_model.model, value, NT, NG)
