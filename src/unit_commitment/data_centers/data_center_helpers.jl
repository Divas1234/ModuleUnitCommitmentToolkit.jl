...
function data_center_workload_profile(DataCentras, time_count::Int, data_center_count::Int; scale::Float64 = 10.0)
	...
	elseif length(tasks) == time_count && data_center_count == 1
		...
		error("Data center computational_power_tasks must be time x center or center x time; got $(size(tasks)), time_count=$time_count, data_center_count=$data_center_count")
	end
end
...
function data_center_time_blocks(time_count::Int; block_count::Int = 6)
	...
	return [(first = (i - 1) * block_size + 1, last = min(i * block_size, time_count)) for i in 1:block_count if (i - 1) * block_size + 1 <= time_count]
end
