...
function add_storage_logic_based_dominance_cuts!(
	...
	for (s, ret) in ret_dic
		...
		if ret.is_feasible == true && has_storage_link_values(ret) && haskey(ret, :storage_dispatch)
			unused_charge = find_unused_active_storage_modes(ret.storage_values.charge, ret.storage_dispatch.charge_power, binary_tolerance, power_tolerance)
			unused_discharge = find_unused_active_storage_modes(ret.storage_values.discharge, ret.storage_dispatch.discharge_power, binary_tolerance, power_tolerance)
			if !isempty(unused_charge) || !isempty(unused_discharge)
				push!(cut_refs, add_storage_mode_dominance_cut!(model, s, ret, x_value, u_value, v_value, unused_charge, unused_discharge; tolerance = binary_tolerance))
			end
		end
	end
...
function add_storage_mode_dominance_cut!(
	...
	mismatch_expr = sum((x_value[i] >= tolerance) ? (1 - x[i]) : x[i] for i in eachindex(x)) +
...
function get_binary_integer_optimality_cut_expression(model::Model, ret, iter_value; scenario_id::Int64 = 1, tolerance::Float64 = 0.5)
	...
	u = model[:u]
	v = model[:v]
	matches_current_solution = sum((x⁽⁰⁾[i] >= tolerance) ? x[i] : (1 - x[i]) for i in eachindex(x)) +
...
function get_master_storage_binary_values(model::Model)
	...
	return (
		charge = value.(model[:κ⁺]),
		discharge = value.(model[:κ⁻]),
		start = value.(model[:α]),
		stop = value.(model[:β]),
	)
end
...
function get_storage_dispatch_values(model::Model)
	...
	if !haskey(object_dict, :pc⁺) || !haskey(object_dict, :pc⁻) || isempty(model[:pc⁺])
		return (
			charge_power = zeros(0, 0),
			discharge_power = zeros(0, 0),
		)
	end
	return (
		charge_power = value.(model[:pc⁺]),
		discharge_power = value.(model[:pc⁻]),
	)
end
...
function get_storage_binary_match_expression(model::Model, ret, scenario_id::Int64, tolerance::Float64)
	...
	matches = sum((ret.storage_values.charge[i] >= tolerance) ? charge[i] : (1 - charge[i]) for i in eachindex(charge)) +
...
"""
function batch_solve_subproblem_with_feasibility_cut(batch_scuc_subproblem_dic::OrderedDict, x, u, v, NG::Int64, NT::Int64, NS::Int64 = 1; storage_binary_values = nothing)
	ret_dic = OrderedDict{Int64, Any}()
	if should_solve_subproblems_in_parallel(NS)
		...
		Threads.@threads for s in 1:NS
			...
			ret_vec[s] = solve_subproblem_with_feasibility_cut(batch_scuc_subproblem_dic[s]::SCUC_Model, x, u, v, NG, NT; scenario_id = s, storage_binary_values = scenario_storage_values)
		end
		for s in 1:NS
			ret_dic[s] = ret_vec[s]
		end
		...
		for s in 1:NS
			scenario_storage_values = select_storage_binary_values_for_subproblem(storage_binary_values, s, batch_scuc_subproblem_dic[s].model)
			ret = solve_subproblem_with_feasibility_cut(batch_scuc_subproblem_dic[s]::SCUC_Model, x, u, v, NG, NT; scenario_id = s, storage_binary_values = scenario_storage_values)
			ret_dic[s] = ret
		end
	end
	return ret_dic
...
function solve_subproblem_with_feasibility_cut(scuc_subproblem_dic::SCUC_Model, x, u, v, NG::Int64, NT::Int64; scenario_id::Int64 = 1, storage_binary_values = nothing)
