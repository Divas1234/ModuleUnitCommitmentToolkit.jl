...
function get_inertiatodamping_functions(droop_parameters)
    ...
    p1 = sub_data_visualization(DAMPING_RANGE, min_inertia, max_inertia, inertia_updown_bindings, extreme_inertia,
        nadir_vector, inertia_vector, selected_ids, min_damping, max_damping, droop, fittingparameters)
...
function sub_data_visualization(damping, min_inertia, max_inertia, inertia_updown_bindings, extreme_inertia, nadir_vector,
        inertia_vector, selected_ids, min_damping, max_damping, droop, fittingparameters)
    ...
    sy1 = Plots.plot(damping, inertia_updown_bindings[:, 1]; framestyle = :box, ylims = (0, maximum(inertia_updown_bindings[:, 1])),
...
function vertices_to_matrix(vertices::AbstractVector)
    ...
    if !all(all(length(v) == first_tuple_length for v ∈ sub_vertices) for sub_vertices ∈ vertices)
