...
function export_overlap_statistics()
    ...
    training_metadata = AdaptiveOverlapTrainingCache.build_case_metadata(;
        ...
        formulation = "single", sampling_version = "hourly-v5-strict-boundary-unknown-initial-candidate3")
    ...
    elseif steady_state_mode == "ml_prediction" && cached_training_data === nothing
        println("  ML cache miss: sampling and training steady-state overlap model before PCM simulation...")
        trained_models = sample_and_train_loss_models(loads, winds, units, lines, DataCentras, config_param, stroges, scenarios_prob,
            hydros, exec_NT, min_overlap, max_overlap, NB, NG, ND, NC, ND2, NL, NH)
        cached_training_data = AdaptiveOverlapTrainingCache.load_cached_dataset(cache_root;
            expected_metadata = training_metadata, min_samples = 4)
        cached_training_data === nothing && error(
            "ML steady-state overlap training did not produce a compatible cache; fallback prediction is disabled.")
    end
    ...
    for k ∈ 1:N_sets
        ...
        x_ref_curr = reference_mode == "load_following" ?
            load_following_commitment(units, maximum(sum(loads.load_curve[:, start_time:(start_time + exec_NT - 1)]; dims = 1))) :
        ...
        selection_time = time() - selection_start
        ...
        push!(df_stats, (
            k, start_time, exec_NT, T_steady, T_unit, is_ramp, T_ramp, raw_max, limiting_factor, T_overlap, total_NT, selection_time, solve_time, status))
    end

    global PCM_SIMULATION_TIME_SEC = sum(df_stats.Overlap_SelectionTime_sec) + sum(df_stats.Subproblem_SolveTime_sec)
    global PCM_ML_TRAINING_TIME_SEC = calibration_time_excluded
    global PCM_OFFLINE_PREPROCESS_TIME_SEC = 0.0
