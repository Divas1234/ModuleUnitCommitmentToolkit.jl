...
function run_offline_dataset_generation(;
        input_file = get(ENV, "MODULE_UC_DATA_FILE", joinpath(ADAPTIVE_PCM_PROJECT_ROOT, "data", "data_118_clustered_pcm.xlsx")),
        load_profile = get(ENV, "PCM_LOAD_PROFILE", "baseline"),
        output_dir = joinpath(ADAPTIVE_PCM_PROJECT_ROOT, "output", "ml_trainningdata_res"),
    ...
    cache_metadata = AdaptiveOverlapTrainingCache.build_case_metadata(;
        ...
        formulation = lowercase(get(ENV, "PCM_TRAINING_FORMULATION", "single")),
        sampling_version = "hourly-v5-strict-boundary-unknown-initial-candidate3")
    cached_training_data = force ? nothing :
                           AdaptiveOverlapTrainingCache.load_cached_dataset(output_dir;
