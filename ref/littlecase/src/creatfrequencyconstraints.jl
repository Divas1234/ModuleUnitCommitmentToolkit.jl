...
function creatfrequencyfittingfunction(units, winds, NG, NW, flag_method_type, whitenoise_parameter)
	Set_f_nadir, set_H, set_δp, set_Dg, set_Fg, set_Kg, set_Rg, sampleStatues = montecalrosimulation(units, winds, NG, NW,
		flag_method_type,
...
function montecalrosimulation(units, winds, NG, NW, flag_method_type, whitenoise_parameter)
	...
	for i in 1:sampleNumber
		...
		f_nadir, t_nadir, H, δp, Kg, Fg, Rg, Dg = creatingfrequencyresponsesamplingdata(units, winds, NW, NG,
			Sampling_Statue, 2, flag_method_type,
			whitenoise_parameter,)
...
function creatingfrequencyresponsesamplingdata(units, winds, NW, NG, Sampling_Statue, flag, flag_method_type,
		whitenoise_parameter,)
	...
	if flag_method_type == 0
		...
		flag, symflag = 1, 0
		seed = rand()
		δf_positor, δf_actual, δf_samplieddata = simulate(generate_data, particle_filter, sample_Num, horizon, flag, symflag,
			units, winds, Sampling_Statue, whitenoise_parameter, seed,)
