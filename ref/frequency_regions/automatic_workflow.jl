include("frequency_parameter_bindings/bounds/inertia_damping_utils.jl")
include("frequency_parameter_bindings/visualizations/inertia_damping_visualization.jl")
...
function get_inertiatodamping_functions(droop_parameters)
	...
	inertia_updown_bindings, extreme_inertia, nadir_vector, inertia_vector, selected_ids = calculate_inertia_parameters(
		initial_inertia, factorial_coefficient, time_constant, droop, power_deviation, DAMPING_RANGE, converter_vsm_parameters, converter_droop_parameters, flag_converter
	)
	...
	p1 = sub_data_visualization(DAMPING_RANGE, min_inertia, max_inertia, inertia_updown_bindings, extreme_inertia, nadir_vector, inertia_vector, selected_ids, min_damping, max_damping, droop, fittingparameters)

	# p1 = data_visualization(DAMPING_RANGE, inertia_updown_bindings, extreme_inertia,
	# 	nadir_vector, inertia_vector, selected_ids)

	vertexs = calculate_vertex(DAMPING_RANGE, inertia_updown_bindings, fittingparameters, min_inertia, max_inertia, min_damping, max_damping, droop)

	return p1, vertexs
end

function sub_data_visualization(damping, min_inertia, max_inertia, inertia_updown_bindings, extreme_inertia, nadir_vector, inertia_vector, selected_ids, min_damping, max_damping, droop, fittingparameters)
	...
	for i in eachindex(damping)
		str = fittingparameters[1] .+ fittingparameters[2] .* damping[i] .+ fittingparameters[3] .* (damping[i] .^ 2)
		if str > min_inertia
			fillarea[i] = str
		else
	...
	@show seq = fittingparameters[1] .+ fittingparameters[2] .* damping .+ fittingparameters[3] .* damping .^ 2 .- max_inertia
	...
	sy1 = Plots.plot(damping, inertia_updown_bindings[:, 1]; framestyle = :box, ylims = (0, maximum(inertia_updown_bindings[:, 1])), xlabel = "damping / p.u.", ylabel = "max inertia / p.u.", lw = 3, label = "upper_bound_1")
	sy1 = Plots.plot!(damping, inertia_updown_bindings[:, 2]; lw = 3, label = "lower_bound_2", color = :forestgreen)
	...
	sy1 = Plots.plot!(damping, fittingparameters[1] .+ fittingparameters[2] .* damping .+ fittingparameters[3] .* damping .^ 2, ; lw = 3)
...
function calculate_vertex(DAMPING_RANGE, inertia_updown_bindings, fittingparameters, min_inertia, max_inertia, min_damping, max_damping, droop)
	...
	function calculate_tem_sequence(fitting_params, damping_range)
		return fitting_params[1] .+ fitting_params[2] .* damping_range .+ fitting_params[3] .* damping_range .^ 2
	end
...
function vertices_to_matrix(vertices::AbstractVector)
	...
	if !all(all(length(v) == first_tuple_length for v in sub_vertices) for sub_vertices in vertices)
