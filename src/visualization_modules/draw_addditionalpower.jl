function calculate_sum_additionalpower(units, winds, Sampling_Statue, bess, δf_positor)
    ...
    current_Kg, current_Rg, current_Kw, current_Rw, current_Dw =
        calculate_diffdynamicparameter(units, winds, Sampling_Statue, 1)
    sum_conventionalunit_power = sum(units.p_max)
    sum_windfarms_power = sum(winds.p_max)
    # sum_bess_power = sum(bench_pss_charge_p⁺)
    ratio_1, ratio_2 =
        sum_conventionalunit_power / (sum_conventionalunit_power + sum_windfarms_power),
    ...
    for t ∈ 1:1201
        conventionalunits_addpower[t, 1] =
            0.5 * current_Kg / current_Rg * δf_positor[t, 1] * ratio_1 / 100 * 2
        windfarms_addpower[t, 1] =
            current_Kw / current_Rw * δf_positor[t, 1] * ratio_2 / 100 * 2
