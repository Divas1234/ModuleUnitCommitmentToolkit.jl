...
function FCUC_scucmodel(NT::Int64, NB::Int64, NG::Int64, ND::Int64, NC::Int64, units::unit,
    loads::load, winds::wind, lines::transmissionline, config_param::config)
    ...
    @constraint(scuc,
        [t = 1:NT],
        sum(winds.Mw[:, 1] .* winds.Fcmode[:, 1] .* winds.p_max[:, 1]) + 2 * sum(x[:, t] .* units.Hg[:, 1] .* units.p_max[:, 1]) >=
    ...
    current_Rw = 1 / sum(winds.Kw .* inverse_winds_Rw .* (ones(NW, 1) - winds.Fcmode) .* winds.p_max) / sum(((ones(NW, 1) - winds.Fcmode) .* winds.p_max))
    ...
    @constraint(scuc,
        [t = 1:NT],
        fittingparameter[1, 1] / sumapparentpower * (sum(x[:, t] .* units.Hg .* units.p_max) + sum(current_Mw .* adjustablewindsVSCpower)) +
        ...
        fittingparameter[1, 4] <= (f_base - f_nadir) * scalling_imbalanced_power)
    ...
    for t in 1:NT
        δf[t,
            1] = fittingparameter[1] / sumapparentpower * (sum(x₀[:, t] .* units.Hg .* units.p_max) + sum(current_Mw .* adjustablewindsVSCpower)) +
