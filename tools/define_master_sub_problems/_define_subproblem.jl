...
Constructs the second-stage recourse subproblem for Benders decomposition. 
This function builds a linear programming (LP) model that optimizes generation 
dispatch, reserves, and flexible resources (Storage, Data Centers) based on 
fixed first-stage commitment decisions.

# Returns
- A tuple: `(scuc_subproblem::Model, sub_scuc_struct::SCUCM_Model)`
"""
...
function bd_subfunction(
    ...
    pss_sumchargeenergy,
    α,
    β = define_subproblem_decision_variables!(
        scuc_subproblem,
        NT,
        NG,
        ND,
    ...
    scuc_subproblem, _winds_curt_constr, _loads_curt_const =
        add_curtailment_constraints!(scuc_subproblem, NT, ND, NW, NS_copy, loads, winds)# Add curtailment constraints for wind and loads
    scuc_subproblem, _units_minpower_constr, _units_maxpower_constr =
        add_generator_power_constraints!(scuc_subproblem, NT, NG, NS_copy, units)# Add generator power constraints
    scuc_subproblem, _sys_upreserve_constr, _sys_down_reserve_constr =
    ...
    scuc_subproblem, _units_upramp_constr, _units_downramp_constr =
        add_ramp_constraints!(scuc_subproblem, NT, NG, NS_copy, units, onoffinit)# Add ramp constraints
    scuc_subproblem,
    _units_pwlpower_sum_constr,
    _units_pwlblock_upbound_constr,
    _units_pwlblock_dwbound_constr =
        add_pwl_constraints!(scuc_subproblem, NT, NG, NS_copy, units)# Add piecewise linear constraints
    scuc_subproblem,
    ...
    # DEBUG - Helper
    all_constraints_dict[:key_winds_curt_constr] =
        vec(collect(Iterators.flatten(_winds_curt_constr)))
    all_constraints_dict[:key_loads_curt_constr] =
        vec(collect(Iterators.flatten(_loads_curt_const)))
    all_constraints_dict[:key_transmissionline_powerflow_upbound_constr] =
        vec(vcat(_transmissionline_powerflow_upbound_constr...))
    all_constraints_dict[:key_transmissionline_powerflow_downbound_constr] =
        vec(vcat(_transmissionline_powerflow_downbound_constr...))
    all_constraints_dict[:key_sys_down_reserve_constr] = vec(_sys_down_reserve_constr)
    all_constraints_dict[:key_units_minpower_constr] =
        vec(collect(Iterators.flatten(_units_minpower_constr)))
    all_constraints_dict[:key_units_maxpower_constr] =
        vec(collect(Iterators.flatten(_units_maxpower_constr)))
    all_constraints_dict[:key_sys_upreserve_constr] = vec(_sys_upreserve_constr)
    all_constraints_dict[:key_units_downramp_constr] =
        vec(collect(Iterators.flatten(_units_downramp_constr)))
    all_constraints_dict[:key_balance_constr] = vec((_sys_balance_constr[1]))
    all_constraints_dict[:key_units_pwlblock_upbound_constr] =
        vec(_units_pwlblock_upbound_constr)
    all_constraints_dict[:key_units_pwlblock_dwbound_constr] =
        vec(_units_pwlblock_dwbound_constr)
    all_constraints_dict[:key_units_pwlpower_sum_constr] = vec(_units_pwlpower_sum_constr)
    all_constraints_dict[:key_units_upramp_constr] =
        vec(collect(Iterators.flatten(_units_upramp_constr)))

    fields = [
        Symbol(string(k)[5:end]) for
    ...
    all_constr_lessthan_sets, all_constr_greaterthan_sets, all_constr_equalto_sets =
...
function get_reorganize_constraints_struct(all_constraints_dict) #depreated
    ...
    all_reorginzed_constraints_dict[:LessThan] =
        collect(Iterators.flatten(all_constr_lessthan_sets))
    all_reorginzed_constraints_dict[:GreaterThan] =
        collect(Iterators.flatten(all_constr_greaterthan_sets))
    all_reorginzed_constraints_dict[:EqualTo] =
        collect(Iterators.flatten(all_constr_equalto_sets))
...
"""
	...
	Adds second-stage (recourse) variables to the subproblem. 
	Note: Commitment variables (x, u, v) are typically fixed to values provided by 
...
"""
    ...
    Defines the subproblem's objective function: minimizing the total expected second-stage 
    operating costs, including fuel costs (generation segments), reserve costs, and 
    penalties for load shedding or wind curtailment.
