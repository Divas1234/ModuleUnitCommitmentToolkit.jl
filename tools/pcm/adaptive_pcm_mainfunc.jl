...
min_overlap = 1                      # Minimum overlap window size (hours)
max_overlap = 12                     # Maximum overlap window size (hours)
slow_unit_threshold = 4.0            # Dwell time threshold (hours) for slow-start units

# Use steady-state accuracy-loss mapping for T_steady.
#
# "ml_prediction" uses a direct overlap predictor and returns before the
# T_steady/T_unit/T_ramp aggregation path. "regression" reaches
# compute_steady_state_overlap_mapping(...), where T_steady is selected as the
# smallest overlap whose predicted accuracy loss is no larger than epsilon.
steady_state_mode = "regression" # Modes: "decay", "regression", "neural_network", or "ml_prediction"

required_horizon = exec_NT * patch_scheduling_ids_numssets
if size(loads.load_curve, 2) < required_horizon
	println("  Extending load curve from $(size(loads.load_curve, 2))h to $(required_horizon)h by repeating the available profile.")
...
if size(winds.scenarios_curve, 2) < required_horizon
	println("  Extending wind scenarios from $(size(winds.scenarios_curve, 2))h to $(required_horizon)h by repeating the available profile.")
...
# Calibrate accuracy-loss mapping before the rolling-horizon simulation loop.
# This is pre-run model preparation; benchmark scripts should not include this
# training/calibration time in the operational simulation solve-time metric.
...
for interval_scheduling_id ∈ 1:patch_scheduling_ids_numssets
	...
	x_ref_curr = solve_local_reference_commitment(
		...
		NB, NG, ND, NC, ND2, NL, NH, interval_scheduling_id
	)
	T_overlap, is_ramp, T_steady, T_unit, T_ramp = compute_adaptive_overlap_window(
		...
		pre_scheduling_results, interval_scheduling_id, steady_state_mode, trained_models, x_ref_curr
	)
