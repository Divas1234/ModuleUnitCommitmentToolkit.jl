...
function draw_SFR_additionalpower(sum_ConventionalUnitsPower_bench, sum_WindPower_bench, sum_Bess_bench, sum_ConventionalUnitsPower_proposed,
        sum_WindPower_proposed, sum_Bess_proposed, sum_ConventionalUnitsPower_enhanced, sum_WindPower_enhanced, sum_Bess_enhanced)
    ...
    p1 = Plots.plot(
        collect(0:0.05:30), sum_ConventionalUnitsPower_proposed[1:601, 1] .+ sum_WindPower_proposed[1:601, 1] .+ sum_Bess_proposed[1:601, 1];
        xlims = (0, 30), ylims = (0, 0.35), size = (300, 300), xlabel = L"t / s", ylabel = L"\textrm{Output \,/\, p.u.}", label = "FDUC",
        # xtickfontsize = 10, ytickfontsize = 10, legendfontsize = 10, xguidefontsize = 10, yguidefontsize = 10, titlefontsize = 10, linealpha = 0.75, ylabelfontsize = 12, xlabelfontsize = 12,
        # foreground_color_legend = nothing,
        tickfontfamily = "Palatino Bold", legendfontfamily = "Palatino Bold", labelfontfamily = "Palatino Bold",
        framestyle = :box, foreground_color_grid = :grey, lw = 3, lc = :orange, la = 0.75,
        # ls = :dash,
        grid = :true, xticks = (collect(0:10:30), collect(0:10:30)))
    ...
    # 	lw = 2.0
    p1 = Plots.plot!(
        collect(0:0.05:60), sum_ConventionalUnitsPower_enhanced[1:1201, 1] .+ sum_WindPower_enhanced[1:1201, 1] .+ sum_Bess_enhanced[1:1201, 1];
        label = "CCFDUC", la = 0.5, lc = :blue, lw = 3.0)
