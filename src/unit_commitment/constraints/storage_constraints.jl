...
"""
function add_storage_constraints!(scuc::Model, NT, NC, NS, config_param, stroges)
	...
	if config_param.is_ConsiderBESS == 0
		...
		@constraint(scuc, [s = 1:NS, t = 1:NT], pc⁺[((s - 1) * NC + 1):(s * NC), t] - ((t == 1) ? P_initial[:, 1] : pc⁺[((s - 1) * NC + 1):(s * NC), t - 1]) .<= gamma_plus[:, 1])
		@constraint(scuc, [s = 1:NS, t = 1:NT], ((t == 1) ? P_initial[:, 1] : pc⁺[((s - 1) * NC + 1):(s * NC), t - 1]) - pc⁺[((s - 1) * NC + 1):(s * NC), t] .<= gamma_minus[:, 1])
		...
		@constraint(scuc, [s = 1:NS, t = 1:NT], qc[((s - 1) * NC + 1):(s * NC), t] .<= Q_max[:, 1]) # Maximum SoC
		@constraint(scuc, [s = 1:NS, t = 1:NT], qc[((s - 1) * NC + 1):(s * NC), t] .>= Q_min[:, 1]) # Minimum SoC
		@constraint(
			scuc,
			[s = 1:NS, t = 1:NT],
		...
		@constraint(scuc, [s = 1:NS], 0.99 * Q_initial[:, 1] .<= qc[((s - 1) * NC + 1):(s * NC), NT] .<= 1.01 * Q_initial[:, 1])

		# Constraints on charging cycles (α, β logic)
		@constraint(scuc, [s = 1:NS, c = 1:NC, t = 1:NT], α[(s - 1) * NC + c, t] >= κ⁺[(s - 1) * NC + c, t] - ((t == 1) ? 0 : κ⁺[(s - 1) * NC + c, t - 1]))
		@constraint(scuc, [s = 1:NS, c = 1:NC, t = 1:NT], β[(s - 1) * NC + c, t] >= ((t == 1) ? 0 : κ⁺[(s - 1) * NC + c, t - 1]) - κ⁺[(s - 1) * NC + c, t])
		...
		return println("\t constraints: 11) stroges system constraints limits\t\t\t done")
	end
end
