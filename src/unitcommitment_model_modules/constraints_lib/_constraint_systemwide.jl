...
export add_curtailment_constraints!, add_reserve_constraints!, add_power_balance_constraints!, add_frequency_constraints!
...
function add_power_balance_constraints!(scuc::Model, NT, NG, ND, NC, NW, NS, loads, winds, config_param, ND2 = nothing) # Added ND2
    ...
    if config_param.is_HydroUnitCon == 1 && ph !== nothing
        common_balance = @expression(scuc, [s = 1:NS, t = 1:NT], common_balance[s, t] + sum(ph[(tem_NH * (s - 1) + 1):(s * tem_NH), t]))
    end
