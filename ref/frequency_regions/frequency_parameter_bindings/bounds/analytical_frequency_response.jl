...
function generate_extreme_inertia(initial_inertia, factorial_coefficient, time_content, droop, delta_p, damping, inertia_updown_bindings, converter_vsm_parameters, converter_droop_parameters, flag_converter::Int64)
	ll = 25

	nadir_vector, inertia_vector, extreme_inertia, selected_ids = zeros(length(damping), ll), zeros(length(damping), ll), zeros(length(damping), 1), zeros(length(damping), 1)
	...
	for i in eachindex(damping)
		...
		for tem_inertia in eachindex(candidate_inertia)
			...
			tem_nadir = calculate_frequencynadir(candidate_inertia[tem_inertia], factorial_coefficient, time_content, droop, delta_p, damping[i], converter_vsm_parameters, converter_droop_parameters, flag_converter::Int64)
...
function calculate_frequencynadir(inertia, factorial_coefficient, time_content, droop, delta_p, damping, converter_vsm_parameters, converter_droop_parameters, flag_converter::Int64)
	...
	if flag_converter == 0
		...
		res = delta_p / (damping + droop) * (1 + (-1)^(k) * sqrt(time_content * (droop - factorial_coefficient) / (2 * inertia)) * exp(-1 * ζ * ωₙ * t))
		...
		ζ = (2 * (combinded_inertia) + time_content * (combinded_damping + factorial_coefficient)) / (2 * sqrt(2 * combinded_inertia * time_content * (combinded_damping + droop)))
		...
		res = delta_p / (combinded_damping + droop) * (1 + (-1)^(k) * sqrt(time_content * (droop - factorial_coefficient) / (2 * combinded_inertia)) * exp(-1 * ζ * ωₙ * t))
	end
	return res
end
