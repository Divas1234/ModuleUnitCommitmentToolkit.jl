...
export readxlssheet, forminputdata, boundrycondition, boundarycondition, boundary_condition, maybe_print_boundarycondition, read_powersystems_case,
       load_uc_data, build_system_from_powersystems, powersystems_case_catalog, list_powersystems_cases, extract_uc_data_from_powersystems,
       generate_wind_scenarios_from_system, load_native_powersystems_case
...
function _normalize_input_source(input, use_powersystems, sys, case_name, case_dir)
    ...
    if use_powersystems !== nothing
        ...
        source != "excel" && source != legacy_source && throw(ArgumentError("input=$(input) conflicts with use_powersystems=$(use_powersystems)"))
...
"""
function load_uc_data(; input::Union{Symbol, AbstractString} = :excel, scenario_limit::Int64 = 50, use_powersystems::Union{Nothing, Bool} = nothing,
        sys = nothing, case_name = nothing, case_category = MatpowerTestSystems, case_dir::String = "",
        data_center_buses::Vector{Int} = Int[], data_center_pmax::Vector{Float64} = Float64[], frequency_params_override = nothing,
        frequency_parameters = frequency_params_override, data_centers = NamedTuple[], horizon::Int64 = 24)
    ...
    if input_source == :powersystems
        if case_name !== nothing
            ...
            return load_native_powersystems_case(String(case_name); case_category = case_category, scenario_limit = scenario_limit,
        ...
        if isempty(case_dir)
            config_param, units, lines, loads, psses, NB, NG, NL, ND, NT, NC, ND2, DataCentras, WindsFreqParam,
            bus_to_idx = extract_uc_data_from_powersystems(sys; data_center_buses = data_center_buses, data_center_pmax = data_center_pmax,
                frequency_parameters = frequency_parameters, data_centers = data_centers, horizon = horizon)
    ...
    elseif input_source == :powersystems_csv
        ...
        config_param, units, lines, loads, psses, winds, NB, NG, NL, ND, NT, NC, ND2, DataCentras = read_powersystems_case(sys, case_dir; scenario_limit = scenario_limit)
    ...
    return (config_param = config_param, units = units, lines = lines, loads = loads, winds = winds, psses = psses, DataCentras = DataCentras,
        NB = NB, NG = NG, NL = NL, ND = ND, NT = NT, NC = NC, ND2 = ND2, NW = NW, NS = NS, full_scenario_probability = 1.0 / NS)
end
