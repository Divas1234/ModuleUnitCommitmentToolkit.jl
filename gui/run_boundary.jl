...
units_data = [
    Dict(
        "index" => units.index[i],
        "bus" => units.locatebus[i],
        "p_max" => units.p_max[i],
        "p_min" => units.p_min[i],
        "ramp_up" => units.ramp_up[i],
        "ramp_down" => units.ramp_down[i],
        "min_up" => units.min_shutup_time[i],
        "min_down" => units.min_shutdown_time[i],
        "init_status" => units.x_0[i],
        "init_power" => units.p_0[i],
        "cost_a" => units.coffi_a[i],
        "cost_b" => units.coffi_b[i],
        "cost_c" => units.coffi_c[i],
    ) for i in 1:NG
]
...
data = Dict(
    "system" => Dict("NB" => NB, "NL" => NL, "NG" => NG, "NT" => NT, "ND" => ND, "NW" => NW, "NS" => NS, "NC" => length(psses.index)),
    "totals" => Dict(
    ...
    "wind" =>
        Dict("count" => NS, "mean_by_hour" => wind_scenario_mean, "installed_capacity" => [round(winds.p_max[i]; digits = 2) for i in 1:NW]),
)
