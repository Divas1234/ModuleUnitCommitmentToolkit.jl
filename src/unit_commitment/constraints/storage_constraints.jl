...
function add_storage_constraints!(scuc::Model, NT, NC, NS, config_param, stroges; include_binary_logic::Bool = true)
    ...
    if config_param.is_ConsiderBESS == 0
        ...
        charge_power_limit = @constraint(scuc, [s = 1:NS, t = 1:NT],
            pc⁺[((s - 1) * NC + 1):(s * NC), t] .<= p_plus[:, 1] .* κ⁺[((s - 1) * NC + 1):(s * NC), t])
        discharge_power_limit = @constraint(scuc, [s = 1:NS, t = 1:NT],
            pc⁻[((s - 1) * NC + 1):(s * NC), t] .<= p_minus[:, 1] .* κ⁻[((s - 1) * NC + 1):(s * NC), t])

        # coupling limits for adjacent discharge/charge constraints (Ramping for storage)
        charge_ramp_up = @constraint(scuc, [s = 1:NS, t = 1:NT],
            pc⁺[((s - 1) * NC + 1):(s * NC), t] - ((t == 1) ? P_initial[:, 1] : pc⁺[((s - 1) * NC + 1):(s * NC), t - 1]) .<= gamma_plus[:, 1])
        charge_ramp_down = @constraint(scuc, [s = 1:NS, t = 1:NT],
        ...
        soc_balance = @constraint(scuc, [s = 1:NS, t = 1:NT],
            qc[((s - 1) * NC + 1):(s * NC), t] .==
        ...
        start_logic = if include_binary_logic
            @constraint(scuc, [s = 1:NS, c = 1:NC, t = 1:NT],
        ...
        start_cycle_limit = if include_binary_logic
            @constraint(scuc, [s = 1:NS, c = 1:NC], sum(α[(s - 1) * NC + c, t] for t ∈ 1:NT) <= 5)
        else
            nothing
        end
        stop_cycle_limit = if include_binary_logic
            @constraint(scuc, [s = 1:NS, c = 1:NC], sum(β[(s - 1) * NC + c, t] for t ∈ 1:NT) <= 5)
        else
            nothing
        end

        println("\t constraints: 11) stroges system constraints limits\t\t\t done")
        return (charge_power_limit = charge_power_limit, discharge_power_limit = discharge_power_limit, charge_ramp_up = charge_ramp_up,
            charge_ramp_down = charge_ramp_down, state_exclusion = state_exclusion, soc_max = soc_max,
            soc_min = soc_min, soc_balance = soc_balance, soc_terminal = soc_terminal, start_logic = start_logic,
            stop_logic = stop_logic, start_cycle_limit = start_cycle_limit, stop_cycle_limit = stop_cycle_limit)
    end
end
