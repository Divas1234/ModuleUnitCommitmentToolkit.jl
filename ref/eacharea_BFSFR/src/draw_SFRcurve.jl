...
function draw_sfr_curve3(xdata, ydata₁, ydata₂, ydata₃, ydata₄, sampledata1, sampledata2)
    ...
    p1 = Plots.plot(xdata[1:1201, 1], ydata₁[1:1201, 1]; color = colorant"#b8b8b8", la = 1, fa = 0.5, ribbon = tem1[:, 2] .* 20, label = false)
    Plots.plot!(xdata[1:1201, 1], ydata₃[1:1201, 1]; color = colorant"#b8b8b8", fa = 0.75, la = 1, ribbon = tem2[:, 2] .* 20, label = false)
    Plots.plot!(
        xdata[1:1201, 1],
        ydata₁[1:1201, 1];
        # size                    = (400, 300),
