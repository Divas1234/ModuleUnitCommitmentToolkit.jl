...
function bd_masterfunction(
                            ...
                            κ⁻ =
                                pc⁺ =
                                    pc⁻ =
                                        qc =
                                            pss_sumchargeenergy =
                                                α = β = Matrix{VariableRef}(undef, 0, 0)
    ...
    all_constraints_dict[:key_units_init_stateslogic_consist_constr] =
        vec(_units_init_stateslogic_consist_constr)
    all_constraints_dict[:key_units_states_consist_constr] =
        vec(_units_states_consist_constr)
    all_constraints_dict[:key_units_init_shutup_cost_constr] =
        vec(_units_init_shutup_cost_constr)
    all_constraints_dict[:key_units_init_shutdown_cost_constr] =
        vec(_units_init_shutdown_cost_constr)
    all_constraints_dict[:key_units_shutup_cost_constr] =
        vec(collect(Iterators.flatten(_units_shutup_cost_constr.data)))
    all_constraints_dict[:key_units_shutdown_cost_constr] =
        vec(collect(Iterators.flatten(_units_shutdown_cost_constr.data)))
...
function define_masterproblem_decision_variables!(
    scuc_masterproblem::Model,
    NT,
    NG,
    ND,
...
function set_masterproblem_objective_economic!(
    ...
    obj = @objective(
        scuc_masterproblem,
        Min,
        sum(sum(su₀[i, t] + sd₀[i, t] for i = 1:NG) for t = 1:NT) + c₀ * θ
    )
