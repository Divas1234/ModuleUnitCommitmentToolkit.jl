...
function add_reserve_constraints!(scuc::Model, NT, NG, NC, NS, units, loads, winds, config_param, hydros = nothing)
    ...
    sys_upreserve_constr = @constraint(
        ...
        (
            ph !== nothing ?
            sum.(hydros.p_max, hydros.reservoircurve[t, 1] * ones(tem_NH, 1)) -
            sum(ph[(tem_NH*(s-1)+1):(s*tem_NH), t]) : 0.0
        ) +
        (NC > 0 && pc⁻ !== nothing ? sum(pc⁻[(NC*(s-1)+1):(s*NC), t]) : 0.0) >= 0.5 * unit_pmax[i, 1] * x[i, t]
...
function add_power_balance_constraints!(scuc::Model, NT, NG, ND, NC, NW, NS, loads, winds, config_param, ND2 = nothing) # Added ND2
    ...
    if config_param.is_HydroUnitCon == 1 && ph !== nothing
        common_balance =
            @expression(scuc, [s = 1:NS, t = 1:NT], common_balance[s, t] + sum(ph[(tem_NH*(s-1)+1):(s*tem_NH), t]))
    end
