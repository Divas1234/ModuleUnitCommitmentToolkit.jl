...
function validate_wind_scenario_data(winds::wind)
    ...
    size(winds.scenarios_curve, 1) == winds.scenarios_nums || throw(
        ArgumentError("wind.scenarios_curve row count ($(size(winds.scenarios_curve, 1))) must match wind.scenarios_nums ($(winds.scenarios_nums))"),
    )
    size(winds.scenarios_curve, 2) > 0 || throw(ArgumentError("wind.scenarios_curve must contain at least one time period"))
    all(isfinite, winds.scenarios_curve) || throw(ArgumentError("wind.scenarios_curve contains NaN or Inf values"))
    all(winds.scenarios_curve .>= 0) || throw(ArgumentError("wind.scenarios_curve contains negative renewable availability values"))
    ...
    all(isfinite, winds.p_max) || throw(ArgumentError("wind.p_max contains NaN or Inf values"))
    all(winds.p_max .>= 0) || throw(ArgumentError("wind.p_max contains negative capacity values"))
    return nothing
end
...
function validate_wasserstein_inputs(costs::Vector{Float64}, nominal_probability::Vector{Float64}, radius::Float64, distance_matrix::Matrix{Float64})
    ...
    all(isfinite, distance_matrix) || throw(ArgumentError("distance matrix contains NaN or Inf values"))
    all(distance_matrix .>= -1e-10) || throw(ArgumentError("distance matrix contains negative entries"))
    isapprox(distance_matrix, transpose(distance_matrix); atol = 1e-8) || throw(ArgumentError("distance matrix must be symmetric"))
    all(abs(distance_matrix[i, i]) <= 1e-8 for i in axes(distance_matrix, 1)) || throw(ArgumentError("distance matrix diagonal must be zero"))
    return probability
end

function worst_case_probabilities(costs::Vector{Float64}, nominal_probability::Vector{Float64}, radius::Float64, distance_matrix::Matrix{Float64})
...
function worst_case_expected_value(costs::Vector{Float64}, nominal_probability::Vector{Float64}, radius::Float64, distance_matrix::Matrix{Float64})
...
function renewable_netload_scores(data)
    ...
    return [
        ...
        0.05 * sum(load_by_time .- data.winds.scenarios_curve[s, :] .* wind_capacity) for s in 1:data.NS
    ]
end
...
function set_dro_ccg_master_objective!(
    ...
    @objective(model, Min, first_stage_cost + radius * dro_lambda + sum(nominal_probability[i] * dro_eta[i] for i in 1:NS_active),)
    return model
end
