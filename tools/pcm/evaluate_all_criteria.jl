...
function run_criteria_evaluation()
    ...
    modes = [
        ("Fixed Overlap (0h)", "fixed_0"),
        ("Fixed Overlap (4h)", "fixed_4"),
        ("Fixed Overlap (12h)", "fixed_12"),
        ("Adaptive (Decay)", "decay"),
        ("Adaptive (ML CART)", "ml_prediction")
    ]

    # Results DataFrame
    df_results = DataFrame(
        Mode = String[],
        SolveTime_sec = Float64[],
        Speedup = Float64[],
        TotalCost_USD = Float64[],
        CostGap_pct = Float64[],
        StartupCost_USD = Float64[],
    ...
    for (name, mode_str) ∈ modes
        ...
        for k ∈ 1:N_sets
            ...
            T_overlap, is_ramp, T_steady, T_unit, T_ramp = compute_adaptive_overlap_window(
                ...
                pre_results, k, mode_str, nothing, x_ref_curr
            )
            ...
            committed_costs = compute_committed_cost(
                committed_res, exec_NT, mini_units, loads, winds, lines,
        ...
        if !isnan(total_cost)
            ...
            push!(df_results, (
                name,
                round(solve_duration, digits=2),
                0.0, # Will fill speedup later
                round(total_cost, digits=2),
                0.0, # Will fill gap later
                round(total_startup_cost, digits=2),
                round(total_wind_curtailment, digits=2),
                round(avg_overlap, digits=1)
            ))
    ...
    if isfile(dataset_path)
        ...
        open(summary_path, "w") do f
            ...
            for col in names(df_data)[1:end-1]
                vals = df_data[:, col]
