...
function generate_data(initialize, trans_fun_1, obs_fun_1, tt, flag, symflag, temflag, whitenoise_parameter, fcr_selected_threshold)
    ...
    for i in 2:tt
        δfₜ_cur = trans_fun_1(trans_fun_0, δf[i - 1, 1], δp_add[i - 1, 1], δt, i, flag, 1, temflag, whitenoise_parameter)
        δf[i] = δfₜ_cur[1]
...
function reconditional_likelihoods(obs_fun_0, δp_measured, δf_inference, t, flag, R, temflag, fcr_selected_threshold)
    R = 1
...
function particle_filter(
    ...
    for t in 2:tt
        ...
        δfₜ_pre = multi_trans_fun_1(trans_fun_0, 𝚫f_post[t - 1, :], 𝚫p_post[t - 1, 1], δt, t - 1, flag, 1, temflag, whitenoise_parameter)
        𝚫f_post[t, :] = δfₜ_pre
        # Update: compute weights p(y_k|x_k)
        likelihood = reconditional_likelihoods(obs_fun_0, y_true[t], 𝚫f_post[1:t, :], t, flag, 1, temflag, fcr_selected_threshold)
...
function simulate(generate_data, particle_filter, sample_Num, horizon, flag, symflag, temflag, seed, whitenoise_parameter, fcr_selected_threshold)
    rand(seed)
