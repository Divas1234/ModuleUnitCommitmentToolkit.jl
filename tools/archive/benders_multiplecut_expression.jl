    ...
    if max(x_order, u_order, v_order) < 0
        if key ∈ index_constr_item
            dual_express = @expression(
                scuc_masterproblem,
                sum(
                    sum(operator_precedence[(t - 1) * NW + w, 1] * dual_coefficient[(t - 1) * NW + w, 1] * rhs[(t - 1) * NW + w, 1] for w in 1:NW) for t in 1:NT
                )
            )
        end
        ...
        if key ∈ index_constr_item
            dual_express = @expression(
                scuc_masterproblem,
                sum(
                    sum(operator_precedence[(t - 1) * ND + d, 1] * dual_coefficient[(t - 1) * ND + d, 1] * rhs[(t - 1) * ND + d, 1] for d in 1:ND) for t in 1:NT
                )
            )
        end
        ...
        if key ∈ index_constr_item
            dual_express = @expression(scuc_masterproblem, sum(operator_precedence[t, 1] * dual_coefficient[t, 1] * rhs[t, 1] for t in 1:NT))
        end
        ...
        if key ∈ index_constr_item
            dual_express = @expression(
                scuc_masterproblem,
                sum(
                    sum(operator_precedence[(t - 1) * NL + l, 1] * dual_coefficient[(t - 1) * NL + l, 1] * rhs[(t - 1) * NL + l, 1] for l in 1:NL) for t in 1:NT
                )
            )
        end
    end
