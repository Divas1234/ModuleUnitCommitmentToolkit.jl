...
function add_datacentra_constraints!(scuc::Model, NT, NS, config_param, ND2 = nothing, DataCentras = nothing)
    ...
    if config_param.is_ConsiderDataCentra == 0
        ...
        @constraint(scuc, [s = 1:NS, t = 1:NT],
            ...
            dc_v²[((s - 1) * ND2 + 1):(s * ND2), t] .+ dc_f[((s - 1) * ND2 + 1):(s * ND2), t] .- ones(ND2, 1)) # Assuming v², f are binary [0,1]
        ...
        @constraint(scuc, [s = 1:NS, t = 1:NT],
            ...
            dc_λ[((s - 1) * ND2 + 1):(s * ND2), t] .+ dc_Δu1[((s - 1) * ND2 + 1):(s * ND2), t] .- ones(ND2, 1)) # Assuming Δu1, λ are binary [0,1]
