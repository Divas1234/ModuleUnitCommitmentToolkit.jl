...
## 最终统一口径复测结果（2026-08-09）

为消除早期实验使用不同 Excel 输入和旧聚类逻辑造成的口径混杂，最终重新运行了 3 种方法 × 3 类负荷曲线共 9 组实验。所有实验统一使用 `data_118.xlsx`、随机种子 `20260809`、3 个 24 h 滚动区间、关闭输电网络约束；自适应方案使用调优后的 `ml_prediction`，离线标定时间为 0 且不计入在线求解时间。

| 负荷曲线 | 方法 | 状态 | 壁钟时间 (s) | Julia 分配量 (MB) | 峰值 RSS (MB) | 总成本 | 事件/窗口 |
|---|---|---|---:|---:|---:|---:|---|
| 原始 | 单机 PCM | OK | 37.79 | 8,760.80 | 1,830.11 | 4,324,770.01 | — |
| 原始 | 聚类 PCM | OK | 37.77 | 7,656.15 | 1,743.46 | 3,452,104.90 | 54→28 等效机组 |
| 原始 | 自适应交叠窗 | OK | 70.72 | 23,353.85 | 2,067.71 | 4,303,561.00 | 0 次；`[10,10,10]` |
| 平滑 | 单机 PCM | OK | 33.97 | 8,768.99 | 1,656.89 | 4,383,325.59 | — |
| 平滑 | 聚类 PCM | OK | 36.24 | 7,668.00 | 1,716.38 | 3,315,258.91 | 54→28 等效机组 |
| 平滑 | 自适应交叠窗 | OK | 49.88 | 23,362.14 | 1,937.01 | 4,916,903.54 | 0 次；`[10,10,10]` |
| 极端爬坡 | 单机 PCM | FAILED | — | — | 1,775.94 | — | 第 3 窗口失败 |
| 极端爬坡 | 聚类 PCM | FAILED | — | — | 1,760.58 | — | 第 3 窗口失败 |
| 极端爬坡 | 自适应交叠窗 | OK | 88.02 | 24,269.52 | 2,139.95 | 4,821,934.03 | 3 次；`[12,12,12]` |

最终结论如下：

1. 自适应交叠窗是唯一完成极端爬坡三窗口调度的方法，且扩窗明确由事件检测器触发。
2. 原始负荷下，自适应方案成本相对单机 PCM 低约 0.49%，但壁钟时间增加约 87%；这是以计算量换取边界鲁棒性。
3. 聚类 PCM 将 54 台机组压缩为 28 个等效机组，Julia 分配量下降约 12.6%–12.7%，原始场景峰值 RSS 下降约 4.7%；目前壁钟加速有限。
4. 聚类成本在原始和平滑场景分别比单机结果低约 20.2% 和 24.4%，明显超出合理求解误差。该差异表明聚合目标成本、解群后单机成本或跨窗状态成本核算仍需统一，不能把低成本解释为精度优势。
5. 因此，当前生产建议仍是：常规曲线使用单机 PCM；存在强爬坡边界风险时使用自适应交叠窗；聚类 PCM 作为实验性降维层，在成本一致性校准完成前保留单机校核或回退。

机器可读的最终数据见 `docs/pcm_final_all_methods_results.csv`。逐窗口运行结果保存在本地 `output/pcm_benchmark/20260809_final_all_methods/`，未将大体积中间文件纳入 Git。

## 前序实验摘要（历史结果）

以下章节保留早期实验数据用于追溯；由于输入文件和聚类实现已经调整，最终判断应以上方“最终统一口径复测结果”为准。
...
4. **`ml_prediction` 已增加物理安全下界并能响应极端爬坡。**调优后采用 `max(T_ml, T_dwell, T_ramp)`，原始和平滑曲线均保持 `[10, 10, 10]` h 且无误报；极端曲线三个区间均检测到事件，动态扩展为 `[12, 12, 12]` h。

### `ml_prediction` 极端爬坡检测调优复测（2026-08-09）

原实现从 CART 预测分支直接返回，绕过了驻留时间和净负荷事件检测。修正后，CART 仅负责精度窗口 `T_ml`，机组剩余驻留时间与极端爬坡窗口继续作为物理安全下界。事件门槛同时采用背景爬坡中位数的 2 倍和平均净负荷的 90%，取较大值；90% 默认值可通过 `PCM_EXTREME_RAMP_RATIO` 调整。

| 场景 | 爬坡事件区间数 | 最终交叠窗 (h) | 壁钟时间 (s) | 峰值 RSS (MB) | 状态 |
|---|---:|---|---:|---:|---|
| 原始 | 0 | `[10, 10, 10]` | 72.47 | 2,079.60 | OK |
| 平滑 | 0 | `[10, 10, 10]` | 55.72 | 1,951.81 | OK |
| 极端爬坡 | 3 | `[12, 12, 12]` | 94.22 | 2,148.67 | OK |

这说明极端场景的额外前瞻现在确实由事件检测器触发，而不是固定窗口偶然提供。完整精简结果见 `docs/pcm_ml_prediction_tuning_results.csv`，逐区间原始输出保存在 `output/pcm_benchmark/20260809_ml_prediction_tuned/`。

### 基线 54 台机组的同质聚类补充验证

聚类主问题不含网络约束时，不再把“同母线”作为同质性的必要条件，而是严格比较容量、最小出力、爬坡、启停爬坡、最小启停时间、成本以及滚动边界初始状态。只有启用网络约束时，才额外要求同母线，以维持节点注入的一致性。

在 `data_118.xlsx` 基线算例中，54 台物理机组形成 28 个等效机组，其中 36 台机组进入 10 个非单机簇，组合状态维度缩减 48.1%。24 h 单区间验证完成纯聚类求解且未触发单机回退。精简结果见 `docs/pcm_baseline_cluster_summary.csv`。
...
- `ml_prediction` 使用 `max(T_ml, T_dwell, T_ramp)`；极端曲线的三个滚动区间均由 `T_ramp=12 h` 主导，原始和平滑曲线仍保持 CART 给出的 10 h。
...
2. **保留已实现的极端事件安全下界。**`ml_prediction` 采用 `max(T_ml, T_dwell, T_ramp)`，生产算例可通过 `PCM_EXTREME_RAMP_RATIO` 标定极端事件门槛，避免普通日内波动导致不必要扩窗。
