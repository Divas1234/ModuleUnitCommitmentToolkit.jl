function get_benders_multicuts_expression(scuc_masterproblem::JuMP.Model, coeff, keys_name, NG, NT, NW, ND, NL, NC = 3)
	...
	if all(x -> x === nothing, (x_order, u_order, v_order))
		...
		if occursin("winds_curt_constr", String(keys_name))
			dual_expression_cut = @expression(scuc_masterproblem, sum(sum((operator_precedence[(t - 1) * NW + w, 1] * dual_coefficient[(t - 1) * NW + w, 1] * rhs[(t - 1) * NW + w, 1]) for w in 1:NW) for t in 1:NT))
		end

		# for load curtailment constraints
		if occursin("loads_curt_constr", String(keys_name))
			dual_expression_cut = @expression(scuc_masterproblem, sum(sum(operator_precedence[(t - 1) * ND + d, 1] * dual_coefficient[(t - 1) * ND + d, 1] * rhs[(t - 1) * ND + d, 1] for d in 1:ND) for t in 1:NT))
		end
		...
		if occursin("units_pwlblock_dwbound_constr", String(keys_name))
			dual_expression_cut = @expression(
				scuc_masterproblem,
				sum(sum(sum(operator_precedence[(NG * NT) * (k - 1) + (t - 1) * NG + g, 1] * dual_coefficient[(NG * NT) * (k - 1) + (t - 1) * NG + g, 1] * rhs[(NG * NT) * (k - 1) + (t - 1) * NG + g, 1] for g in 1:NG) for t in 1:NT) for k in 1:NC)
			)
		end

		# for system balance_constr
		if occursin("balance_constr", String(keys_name)) || occursin("sys_down_reserve_constr", String(keys_name))
			dual_expression_cut = @expression(scuc_masterproblem, sum(operator_precedence[t, 1] * dual_coefficient[t, 1] * rhs[t, 1] for t in 1:NT))
		end
		...
		if occursin("transmissionline_powerflow_upbound_constr", String(keys_name))
			dual_expression_cut = @expression(scuc_masterproblem, sum(sum(operator_precedence[(t - 1) * NL + l, 1] * dual_coefficient[(t - 1) * NL + l, 1] * rhs[(t - 1) * NL + l, 1] for l in 1:NL) for t in 1:NT))
		end
		...
		if occursin("units_pwlblock_upbound_constr", String(keys_name))
			dual_expression_cut = @expression(
				...
				sum(sum(sum((operator_precedence[t, 1] * dual_coefficient[t, 1] * rhs[t, 1]) +
							(if x_order == 0
								(dual_coefficient[(NG * NT) * (k - 1) + (t - 1) * NG + g, 1] * operator_precedence[(NG * NT) * (k - 1) + (t - 1) * NG + g, 1] * x_coefficient[(t - 1) * NG + g, 1] * scuc_masterproblem[:x][g, t])
							else
								(dual_coefficient[(g - 1) * NT + t, 1] * operator_precedence[(NG * NT) * (k - 1) + (g - 1) * NT + t, 1] * x_coefficient[(g - 1) * NT + t, 1] * scuc_masterproblem[:x][g, t])
							end) +
							(if x_order == 0
								(dual_coefficient[(NG * NT) * (k - 1) + (t - 1) * NG + g, 1] * operator_precedence[(NG * NT) * (k - 1) + (t - 1) * NG + g, 1] * rhs[(NG * NT) * (k - 1) + (t - 1) * NG + g, 1])
							else
								(dual_coefficient[(NG * NT) * (k - 1) + (g - 1) * NT + t, 1] * operator_precedence[(NG * NT) * (k - 1) + (g - 1) * NT + t, 1] * rhs[(NG * NT) * (k - 1) + (g - 1) * NT + t, 1])
		...
		if occursin("units_upramp_constr", String(keys_name))
			dual_expression_cut = @expression(
				...
				sum(sum((if isnothing(x_order)
							...
							(if x_order == 0
								(dual_coefficient[(t - 1) * NG + g, 1] * operator_precedence[(t - 1) * NG + g, 1] * x_coefficient[(t - 1) * NG + g, 1] * (1) * ((t == 1) ? 0 : scuc_masterproblem[:x][g, t - 1]))
							else
								(dual_coefficient[(g - 1) * NT + t, 1] * operator_precedence[(g - 1) * NT + t, 1] * x_coefficient[(g - 1) * NT + t, 1] * (1) * ((t == 1) ? 0 : scuc_masterproblem[:x][g, t - 1]))
							end)
						end) +
						(if isnothing(u_order)
							0
							...
							(if u_order == 0
								(dual_coefficient[(t - 1) * NG + g, 1] * operator_precedence[(t - 1) * NG + g, 1] * u_coefficient[(t - 1) * NG + g, 1] * (1) * ((t == 1) ? 0 : scuc_masterproblem[:x][g, t - 1]))
							else
								(dual_coefficient[(g - 1) * NT + t, 1] * operator_precedence[(g - 1) * NT + t, 1] * u_coefficient[(g - 1) * NT + t, 1] * (1) * ((t == 1) ? 0 : scuc_masterproblem[:x][g, t - 1]))
							end)
						end) +
						(if isnothing(v_order)
							0
							...
							(if v_order == 0
								(dual_coefficient[(t - 1) * NG + g, 1] * operator_precedence[(t - 1) * NG + g, 1] * v_coefficient[(t - 1) * NG + g, 1] * (1) * ((t == 1) ? 0 : scuc_masterproblem[:x][g, t - 1]))
							else
								(dual_coefficient[(g - 1) * NT + t, 1] * operator_precedence[(g - 1) * NT + t, 1] * v_coefficient[(g - 1) * NT + t, 1] * (1) * ((t == 1) ? 0 : scuc_masterproblem[:x][g, t - 1]))
							end)
						end) +
						((if isnothing(x_order)
							(dual_coefficient[(t - 1) * NG + g, 1] * operator_precedence[(t - 1) * NG + g, 1] * rhs[(t - 1) * NG + g, 1])
