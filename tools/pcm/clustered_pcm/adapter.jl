if !isdefined(@__MODULE__, :_PCM_CLUSTERED_ADAPTER_INCLUDED)
	...
	function _pcm_initial_states(c, units)
		[begin
			 on=units.x_0[i]>0.5
			 # t_0/t_1 are elapsed hours in the current state.  The residence-flow
			 # checker also consumes elapsed duration; converting it to remaining
			 # dwell time here made mature units look immature in later PCM windows.
			 raw_elapsed=on ? units.t_0[i] : units.t_1[i]
			 minimum=on ? c.min_up : c.min_down
			 # Zero in the workbook means unavailable history, matching the
			 # physical single-unit model: assume a mature initial state.
			 elapsed=raw_elapsed > 0 ? raw_elapsed : minimum
			 InitialUnitState(; unit = i, on = on, duration = max(0, round(Int, elapsed)))
		 end
		...
		[begin
			 ...
			 PhysicalUnitData(; id = i, cluster = c.id, bus = Int(units.locatebus[i]), p_min = Float64(units.p_min[i]),
				 ...
				 initial_duration = begin
					 on=units.x_0[i]>0.5
					 raw=on ? units.t_0[i] : units.t_1[i]
					 max(0, round(Int, raw > 0 ? raw : (on ? c.min_up : c.min_down)))
				 end,
				 initial_power = Float64(units.p_0[i]),
	...
	function _pcm_fallback_paths(c, x, y, z, units)
		...
		for (local_id, i) ∈ enumerate(c.unit_indices)
			...
			raw_age=on0 ? units.t_0[i] : units.t_1[i]
			age=max(0, round(Int, raw_age > 0 ? raw_age : minimum))
			ages=Int[]
			states=Symbol[]
			for t ∈ axes(x, 2)
