...
function bd_subfunction(
    ...
    scuc_subproblem, _sys_upreserve_constr, _sys_down_reserve_constr = add_reserve_constraints!(scuc_subproblem, NT, NG, NC, NS_copy, units, loads, winds, config_param)
    ...
    _transmissionline_powerflow_downbound_constr = add_transmission_constraints!(scuc_subproblem, NT, NG, ND, NC, NW, NL, NS_copy, units, loads, winds, lines, psses, gsdf, config_param, ND2, DataCentras)# Add transmission constraints
    ...
    fields = [Symbol(string(k)[5:end]) for k ∈ keys(all_constraints_dict) if startswith(string(k), "key_")]
    sub_cons = build_constraints(; (f => all_constraints_dict[Symbol("key_", f)] for f ∈ fields)...,)
...
function get_reorganize_constraints_struct(all_constraints_dict) #depreated
    ...
    sub_reformat_cons = SCUCModel_reformat_constraints([vec(all_reorginzed_constraints_dict[key]) for key ∈ [:EqualTo, :GreaterThan, :LessThan]]...,)
...
function set_subproblem_objective_economic!(scuc_subproblem::Model, NT::Int64, NG::Int64, ND::Int64, NW::Int64, NS::Int64, units::unit, config_param::config, scenarios_prob::Float64)
    ...
    obj = @objective(scuc_subproblem, Min,
        # sum(sum(su₀[i, t] + sd₀[i, t] for i in 1:NG) for t in 1:NT)+
        pₛ *
        c₀ *
        (sum(sum(sum(sum(pgₖ[i + (s - 1) * NG, t, :] .* eachslope[:, i] for t ∈ 1:NT)) for s ∈ 1:NS_copy) for i ∈ 1:NG) +
         sum(sum(sum(x[:, t] .* refcost[:, 1] for t ∈ 1:NT)) for s ∈ 1:NS_copy) +
         sum(sum(sum(ρ⁺ * sr⁺[i + (s - 1) * NG, t] + ρ⁻ * sr⁻[i + (s - 1) * NG, t] for i ∈ 1:NG) for t ∈ 1:NT) for s ∈ 1:NS_copy))+pₛ*load_curtailment_penalty*sum(sum(sum(Δpd[(1 + (s - 1) * ND):(s * ND), t]) for t ∈ 1:NT) for s ∈ 1:NS_copy)+pₛ*wind_curtailment_penalty*sum(sum(sum(Δpw[
                                                                                                                                                                                                                                                                                         (1 + (s - 1) * NW):(s * NW), t])
                                                                                                                                                                                                                                                                                 for t ∈ 1:NT)
        for s ∈ 1:NS_copy))
