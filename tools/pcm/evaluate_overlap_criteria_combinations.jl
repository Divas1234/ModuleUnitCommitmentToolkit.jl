...
function selected_overlap_from_components(criteria::Tuple{Bool, Bool, Bool}, T_steady::Int, T_unit::Int, T_ramp::Int, min_overlap::Int, max_overlap::Int, start_time::Int, exec_NT::Int, total_time_avail::Int)
...
function run_mode!(
        df_results::DataFrame, df_intervals::DataFrame, mode_name::String,
        criteria::Tuple{Bool, Bool, Bool}, loads, winds, units, lines, DataCentras,
        config_param, stroges, scenarios_prob, hydros, trained_models, x_refs,
        exec_NT::Int, N_sets::Int, alpha::Float64, epsilon::Float64,
        min_overlap::Int, max_overlap::Int, NB::Int, NG::Int, ND::Int, NC::Int,
        ND2::Int, NL::Int, NH::Int)
    ...
    elapsed = @elapsed allocated_bytes = @allocated begin
        for k in 1:N_sets
            ...
            _, is_ramp, T_steady, T_unit, T_ramp = compute_adaptive_overlap_window(
                loads, winds, units, start_time, exec_NT, alpha, epsilon,
                min_overlap, max_overlap, pre_results, k, "regression",
                trained_models, x_refs[k]
            )
            T_overlap, raw_overlap = selected_overlap_from_components(
                criteria, T_steady, T_unit, T_ramp, min_overlap, max_overlap,
                start_time, exec_NT, total_time_avail
            )
            ...
            res = each_period_scucmodel_modules(
                ...
                lines, DataCentras, config_param, stroges, scenarios_prob, NL, k,
                hydros, NH
            )
            ...
            push!(df_intervals, (
                mode_name, k, start_time, T_steady, T_unit, T_ramp, is_ramp,
                raw_overlap, T_overlap, limiting_factor, total_NT, solve_time
            ))
    ...
    push!(df_results, (
        ...
        criteria[3],
        total_cost,
        total_startup_shutdown,
        total_fuel,
        total_load_shedding,
        total_wind_curtailment,
        elapsed,
        sum(solve_times),
        allocated_bytes / 1024.0^2,
        mean(overlap_history),
        maximum(overlap_history),
        minimum(overlap_history),
        join(["$k=$v" for (k, v) in sort(collect(factor_counts))], "; ")
    ))
end
...
function main()
    ...
    calibration_time = @elapsed trained_models = sample_and_train_loss_models(
        ...
        scenarios_prob, hydros, exec_NT, min_overlap, max_overlap,
        NB, NG, ND, NC, ND2, NL, NH
    )
    ...
    modes = [
        ("NoOverlap", (false, false, false)),
        ("SteadyOnly", (true, false, false)),
        ("UnitOnly", (false, true, false)),
        ("RampOnly", (false, false, true)),
        ("Steady+Unit", (true, true, false)),
        ("Steady+Ramp", (true, false, true)),
        ("Unit+Ramp", (false, true, true)),
        ("Steady+Unit+Ramp", (true, true, true)),
    ]
    ...
    df_results = DataFrame(
        ...
        StartupShutdownCost_USD = Float64[],
        FuelCost_USD = Float64[],
        LoadShedding_MWh = Float64[],
        WindCurtailment_MWh = Float64[],
        WallTime_sec = Float64[],
        SubproblemSolveTime_sec = Float64[],
        JuliaAllocated_MB = Float64[],
        AvgOverlap_h = Float64[],
        MaxOverlap_h = Int[],
        MinOverlap_h = Int[],
        LimitingFactorCounts = String[],
    )

    df_intervals = DataFrame(
        Mode = String[],
        Interval_ID = Int[],
        Start_Hour = Int[],
        T_steady = Int[],
        ...
        Raw_Selected_Overlap_h = Int[],
        Final_Overlap_h = Int[],
        Limiting_Factor = String[],
        Total_Solved_Horizon_h = Int[],
        Subproblem_SolveTime_sec = Float64[],
    )

    for (mode_name, criteria) in modes
        println("-"^80)
        println("Running mode: $mode_name")
        run_mode!(
            df_results, df_intervals, mode_name, criteria, loads, winds, units,
            lines, DataCentras, config_param, stroges, scenarios_prob, hydros,
            trained_models, x_refs, exec_NT, N_sets, alpha, epsilon,
            min_overlap, max_overlap, NB, NG, ND, NC, ND2, NL, NH
        )
    end
