...
function genscenario(WindsFreqParam, flag, NT = 24)
    # The Excel frequency table defines the wind-farm count. Previously this
    # was hard-coded to two farms, so additional spreadsheet rows were ignored.
    NW = max(1, size(WindsFreqParam, 1))
    index = collect(1:NW)
    locatebus = fill(1, NW)
    p_max = fill(2.5, NW)
    if flag == 1
        ...
        # Use the same configurable seed as PCM so independent benchmark runs
        # receive exactly the same stochastic wind trajectory.
        Random.seed!(parse(Int, get(ENV, "PCM_RANDOM_SEED", "20260809")))
        scenarios_nums = 5
