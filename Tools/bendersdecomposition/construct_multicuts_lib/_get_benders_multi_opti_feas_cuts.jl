function add_optimitycut_constraints!(
		scuc_masterproblem::JuMP.Model, sub_model_struct::SCUC_Model, ret, iter_value
)
	...
	add_optimity_cut = @constraint(scuc_masterproblem,
		...
		ret.θ + sum(
		ret.ray_x .* (scuc_masterproblem[:x] - x⁽⁰⁾) +
		ret.ray_u .* (scuc_masterproblem[:u] - u⁽⁰⁾) +
...
function add_benders_multicuts_constraints!(
	...
	scuc_masterproblem, benders_cut = get_benders_cumulative_multicuts_expression(
		scuc_masterproblem, dual_coeffs, NG, NT, NW, ND, NL
	)
