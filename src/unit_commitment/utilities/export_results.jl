...
function write_startup_shutdown_decisions_csv(path, units, u, v, NT)
    ...
    rows = (Any[t, [u[g, t] for g in 1:length(units.index)]..., [v[g, t] for g in 1:length(units.index)]...] for t in 1:NT)
...
function write_startup_shutdown_costs_csv(path, units, su_cost, sd_cost, NT)
    ...
    rows = (Any[t, [su_cost[g, t] for g in 1:length(units.index)]..., [sd_cost[g, t] for g in 1:length(units.index)]...] for t in 1:NT)
...
function write_reserve_schedule_csv(path, units, reserve_up, reserve_down, NT, NG, NS)
    rows = (
        Any[s, units.index[g], t, reserve_up[scenario_row(NG, s, g), t], reserve_down[scenario_row(NG, s, g), t]] for s in 1:NS, g in 1:NG, t in 1:NT
    )
...
function write_curtailment_schedule_csv(path, loads, winds, load_curtailment, wind_curtailment, NT, ND, NW, NS)
    ...
    rows = (
        Any[s, t, [load_curtailment[scenario_row(ND, s, d), t] for d in 1:ND]..., [wind_curtailment[scenario_row(NW, s, w), t] for w in 1:NW]...] for
        s in 1:NS, t in 1:NT
    )
...
function write_power_balance_summary_csv(
    ...
    for s in 1:NS, t in 1:NT
        ...
        data_center_load = if (config_param.is_ConsiderDataCentra == 1 && ND2 > 0 && dc_power !== nothing)
            sum(dc_power[scenario_row(ND2, s, dc), t] for dc in 1:ND2)
        else
            0.0
        end
        net_balance =
            thermal_generation + wind_available - wind_spill + bess_discharge_total - bess_charge_total - (load_total - load_shed) - data_center_load
        push!(
            rows,
            Any[
...
function write_bess_schedule_csv(path, charge_state, discharge_state, charge_power, discharge_power, energy, NT, NC, NS)
    rows = (
        Any[
            ...
            discharge_power[scenario_row(NC, s, c), t],
            energy[scenario_row(NC, s, c), t],
        ] for s in 1:NS, c in 1:NC, t in 1:NT
    )
    return write_rows_csv(
        path,
        ["scenario", "storage_id", "time", "charge_state", "discharge_state", "charge_power", "discharge_power", "stored_energy"],
        rows,
    )
end
...
function write_data_center_schedule_csv(path, DataCentras, dc_power, fv2, fv2lambda, fv2_plus, fv2_minus, fv2lambda_plus, NT, ND2, NS)
    rows = (
        Any[
            s,
            DataCentras.index[dc],
            t,
            dc_power[scenario_row(ND2, s, dc), t],
            fv2[scenario_row(ND2, s, dc), t],
            fv2lambda[scenario_row(ND2, s, dc), t],
            fv2_plus[scenario_row(ND2, s, dc), t],
            fv2_minus[scenario_row(ND2, s, dc), t],
            fv2lambda_plus[scenario_row(ND2, s, dc), t],
        ] for s in 1:NS, dc in 1:ND2, t in 1:NT
    )
