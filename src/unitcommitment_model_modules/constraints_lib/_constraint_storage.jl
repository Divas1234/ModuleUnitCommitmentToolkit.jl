...
function add_storage_constraints!(scuc::Model, NT, NC, NS, config_param, stroges)
    ...
    if config_param.is_ConsiderBESS == 0
        ...
        @constraint(scuc, [s = 1:NS, t = 1:NT], pc⁺[((s - 1) * NC + 1):(s * NC), t] .<= p_plus[:, 1] .* κ⁺[((s - 1) * NC + 1):(s * NC), t]) # charge power
        @constraint(scuc, [s = 1:NS, t = 1:NT], pc⁻[((s - 1) * NC + 1):(s * NC), t] .<= p_minus[:, 1] .* κ⁻[((s - 1) * NC + 1):(s * NC), t]) # discharge power
        ...
        @constraint(scuc, [s = 1:NS, t = 1:NT], qc[((s - 1) * NC + 1):(s * NC), t] .<= Q_max[:, 1])
        @constraint(scuc, [s = 1:NS, t = 1:NT], qc[((s - 1) * NC + 1):(s * NC), t] .>= Q_min[:, 1])
        @constraint(scuc, [s = 1:NS, t = 1:NT],
            qc[((s - 1) * NC + 1):(s * NC), t] .==
            ((t == 1) ? Q_initial[:, 1] : qc[((s - 1) * NC + 1):(s * NC), t - 1]) + # Use Q₀ for initial SoC
            eta_plus[:, 1] .* pc⁺[((s - 1) * NC + 1):(s * NC), t] - (ones(NC, 1) ./ eta_minus[:, 1]) .* pc⁻[((s - 1) * NC + 1):(s * NC), t]) # Assuming Δt=1hr implicit
