...
function data_visualization(
    ...
    plot!(sp1, damping, extreme_inertia; fillrange = inertia_updown_bindings[:, 1], fillalpha = 0.3, label = "Inertia Range", color = :skyblue)
...
function plot_inertia_distribution_with_bounds_improved(
    ...
    hline!(plot_middle, [min_inertia]; lw = common_line_width, label = "Min Inertia", linestyle = :dash, color = :black)
    plot!(plot_middle, damping, max_inertia; lw = common_line_width, label = "Max Inertia", color = :orange, linestyle = :dashdot)
