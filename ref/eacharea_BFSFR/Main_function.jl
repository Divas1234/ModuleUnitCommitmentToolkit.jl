...
function main()
	...
	p1 = draw_sfr_curve3(xdata, results["converter_not_little_noise"].y2, results["converter_not_little_noise"].y3,
		results["converter_yes_little_noise"].y2, results["converter_yes_little_noise"].y3,
		results["converter_not_little_noise"].s_data, results["converter_yes_little_noise"].s_data,)
	p2 = draw_sfr_curve3(xdata, results["converter_not_big_noise"].y2, results["converter_not_big_noise"].y3,
		results["converter_yes_big_noise"].y2, results["converter_yes_big_noise"].y3,
		results["converter_not_big_noise"].s_data, results["converter_yes_big_noise"].s_data,)
	...
	for i in 1:N
		ydata[:, i], ~, sampled_filters[:, :, i] = simulate(generate_data, particle_filter, 100, 60, i, 1, 1, 1234, whitenoise_parameter, fcr_selected_threshold)
	end
