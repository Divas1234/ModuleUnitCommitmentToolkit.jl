...
function add_storage_constraints!(scuc::Model, NT, NC, NS, config_param, stroges; include_binary_logic::Bool = true)
	...
	if config_param.is_ConsiderBESS == 0
		...
		charge_power_limit = @constraint(scuc, [s = 1:NS, t = 1:NT], pc⁺[((s - 1) * NC + 1):(s * NC), t] .<= p_plus[:, 1] .* κ⁺[((s - 1) * NC + 1):(s * NC), t])
		discharge_power_limit = @constraint(scuc, [s = 1:NS, t = 1:NT], pc⁻[((s - 1) * NC + 1):(s * NC), t] .<= p_minus[:, 1] .* κ⁻[((s - 1) * NC + 1):(s * NC), t])

		# coupling limits for adjacent discharge/charge constraints (Ramping for storage)
		charge_ramp_up = @constraint(scuc, [s = 1:NS, t = 1:NT], pc⁺[((s - 1) * NC + 1):(s * NC), t] - ((t == 1) ? P_initial[:, 1] : pc⁺[((s - 1) * NC + 1):(s * NC), t - 1]) .<= gamma_plus[:, 1])
		charge_ramp_down = @constraint(scuc, [s = 1:NS, t = 1:NT], ((t == 1) ? P_initial[:, 1] : pc⁺[((s - 1) * NC + 1):(s * NC), t - 1]) - pc⁺[((s - 1) * NC + 1):(s * NC), t] .<= gamma_minus[:, 1])

		# Mutual exclusion constraints in charge and discharge states
		state_exclusion = include_binary_logic ? @constraint(scuc, [s = 1:NS, t = 1:NT, c = 1:NC], κ⁺[(s - 1) * NC + c, t] + κ⁻[(s - 1) * NC + c, t] <= 1) : nothing
		...
		soc_balance = @constraint(scuc,
			[s = 1:NS, t = 1:NT],
			qc[((s - 1) * NC + 1):(s * NC), t] .== ((t == 1) ? Q_initial[:, 1] : qc[((s - 1) * NC + 1):(s * NC), t - 1]) + eta_plus[:, 1] .* pc⁺[((s - 1) * NC + 1):(s * NC), t] - (ones(NC, 1) ./ eta_minus[:, 1]) .* pc⁻[((s - 1) * NC + 1):(s * NC), t])
		...
		start_logic = include_binary_logic ? @constraint(scuc, [s = 1:NS, c = 1:NC, t = 1:NT], α[(s - 1) * NC + c, t] >= κ⁺[(s - 1) * NC + c, t] - ((t == 1) ? 0 : κ⁺[(s - 1) * NC + c, t - 1])) : nothing
		stop_logic = include_binary_logic ? @constraint(scuc, [s = 1:NS, c = 1:NC, t = 1:NT], β[(s - 1) * NC + c, t] >= ((t == 1) ? 0 : κ⁺[(s - 1) * NC + c, t - 1]) - κ⁺[(s - 1) * NC + c, t]) : nothing
