using Pkg
Pkg.activate("./.pkg")
# Pkg.add(["Revise", "JuMP", "Gurobi", "Test", "DelimitedFiles", "PlotlyJS",
# "LaTeXStrings", "Plots", "JLD", "DataFrames", "Clustering", "StatsPlots"])
using PkgServerClient
PkgServerClient.set_mirror("NJU")

using Revise, JuMP, Gurobi, Test, DelimitedFiles, LaTeXStrings, Plots, DataFrames, Clustering, StatsPlots
...
config_param, units, lines, loads, stroges, NB, NG, NL, ND, NT, NC = forminputdata(DataGen, DataBranch, DataLoad, LoadCurve,
	GenCost, UnitsFreqParam, StrogeData,)
...
e_x₀, e_p₀, e_pᵨ, e_pᵩ, e_seq_sr⁺, e_seq_sr⁻, e_pss_charge_p⁺, e_pss_charge_p⁻, e_su_cost, e_sd_cost, e_prod_cost, e_cost_sr⁺, e_cost_sr⁻ = enhance_FCUC_scucmodel(NN, NT, NB, NG, ND, NC, units, loads, winds, lines, config_param, fittingparameter_vector)
...
bench_x₀, bench_p₀, bench_pᵨ, bench_pᵩ, bench_seq_sr⁺, bench_seq_sr⁻, bench_pss_charge_p⁺, bench_pss_charge_p⁻,
bench_su_cost, bench_sd_cost, bench_prod_cost, bench_cost_sr⁺, bench_cost_sr⁻ = SUC_scucmodel(NT, NB, NG, ND, NC, units, loads, winds, lines, config_param)
# savebalance_result(
...
if Sys.iswindows()
	filepath = "C:/Users/yyp_uestc/Downloads/unitcommitment_code-master/littlecase/fig/"
else
	filepath = "/Users/yuanyiping/Documents/GitHub/RfcucCaseStudies/littlecase/fig/"
end
...
# NOTE cost
nam = repeat([L"\textrm{Shut-up\,\,\, cost}",
		L"\textrm{Shut-off\,\,\, cost}", L"\textrm{Fuel\,\,\, cost}",];
	outer = 2,)
ctg = repeat(["FDUC", "TUC"]; inner = 3)
res = transpose([e_su_cost, e_sd_cost, e_prod_cost; su_cost, sd_cost, prod_cost;
	bench_su_cost, bench_sd_cost, bench_prod_cost * 1.05,])
fig4 = StatsPlots.groupedbar(nam,
	...
	bar_width = 0.75,
	group = ctg,
	xtickfontsize = 10, ytickfontsize = 10, legendfontsize = 10, xguidefontsize = 10,
	yguidefontsize = 10,
...
p5 = groupedbar(nam, -[enhance_rocof_distribution rocof_distribution bench_rocof_distribution] / 10;
	...
	palette = :Paired_3,)
# gr()
Plots.plot(enhance_nadir_distribution .* 0.85)
Plots.plot!(nadir_distribution)
p6 = groupedbar(nam, -[-enhance_nadir_distribution .* 0.85 -nadir_distribution -bench_nadir_distribution];
	group            = sx,
	xlims            = (0, 25),
	ylims            = (-1.0, 0.50),
	size             = (300, 300),
...
fig7 = Plots.plot(p5, p6; legend_column = 1, layout = (1, 2), size = (600, 300))
Plots.savefig(p5, filepath * "ROCOF.svg")
Plots.savefig(p6, filepath * "NADIR.svg")

if Sys.iswindows()
	file_text_path = "C:/Users/yyp_uestc/Downloads/unitcommitment_code-master/littlecase/output/RocofandNadir_distribution/"
else
	file_text_path = "/Users/yuanyiping/Documents/GitHub/RfcucCaseStudies/littlecase/output/RocofandNadir_distribution/"
end
filenames = ["enhance_nadir_distribution.txt",
...
distributions = [enhance_nadir_distribution,
	nadir_distribution,
...
for (filename, distribution) in zip(filenames, distributions)
	open(file_text_path * filename, "w") do file
		return writedlm(file, distribution)
	end
end
...
Plots.savefig(fig7,
	filepath *
	"RoCOFandnadirdistributionINFO.pdf",)
...
fig6 = Plots.plot(collect(0:0.05:60), -bench_δf_positor[1:1201, 1];
	lc = colorant"#000000", la = 0.5, lw = 3.0, label = "TUC",)
...
fig6 = Plots.plot!(collect(0:0.05:60), -enhance_δf_positor[1:1201, 1];
	lc = colorant"#a00000", la = 0.5, lw = 3.0, label = "r-FCUC",)
...
sum_ConventionalUnitsPower_bench, sum_WindPower_bench, sum_Bess_bench = calculate_sum_additionalpower(units, winds,
	Sampling_Statue,
	stroges,
	bench_δf_positor,)
sum_ConventionalUnitsPower_proposed, sum_WindPower_proposed, sum_Bess_proposed = calculate_sum_additionalpower(units, winds,
	Sampling_Statue,
	stroge,
	δf_positor,)
sum_ConventionalUnitsPower_enhanced, sum_WindPower_enhanced, sum_Bess_enhanced = calculate_sum_additionalpower(units, winds,
	Sampling_Statue,
	stroges,
	enhance_δf_positor,)
...
Plots.savefig(fig8, filepath * "SFRcurveandadditionaPower.pdf")

using CSV
bench = hcat(sum_ConventionalUnitsPower_bench, sum_WindPower_bench, sum_Bess_bench)
proposed = hcat(sum_ConventionalUnitsPower_proposed, sum_WindPower_proposed, sum_Bess_proposed)
enhanced = hcat(sum_ConventionalUnitsPower_enhanced, sum_WindPower_enhanced, sum_Bess_enhanced)
filepath = "D:\\GithubClonefiles\\RFCUC\\RfcucCaseStudies\\littlecase\\output\\check_additionapower_frequencydynamics"
CSV.write(joinpath(filepath, "bench.csv"), DataFrames.DataFrame(bench, :auto))
CSV.write(joinpath(filepath, "proposed.csv"), DataFrames.DataFrame(proposed, :auto))
CSV.write(joinpath(filepath, "enhanced.csv"), DataFrames.DataFrame(enhanced, :auto))

frequencydynamic = hcat(-bench_δf_positor, -δf_positor, -enhance_δf_positor)
CSV.write(joinpath(filepath, "frequencydynamic.csv"), DataFrames.DataFrame(frequencydynamic, :auto))
...
p1 = draw_powerbalance(bench_x₀, bench_p₀, bench_pᵨ, bench_pᵩ, bench_seq_sr⁺, bench_seq_sr⁻,
	bench_pss_charge_p⁺, bench_pss_charge_p⁻, bench_su_cost, bench_sd_cost,
	bench_prod_cost, bench_cost_sr⁺, bench_cost_sr⁻, preselected_scenarios,)
p2 = draw_powerbalance(x₀, p₀, pᵨ, pᵩ, seq_sr⁺, seq_sr⁻, pss_charge_p⁺, pss_charge_p⁻,
	su_cost, sd_cost, prod_cost, cost_sr⁺, cost_sr⁻, preselected_scenarios,)
