...
function add_storage_constraints!(scuc::Model, NT, NC, NS, config_param, stroges)
    ...
    if NC == 0 || check_var_exists(scuc, "pc⁺") # Check if variables were defined
        println(
            "\t constraints: 11) stroges system constraints skipped (NC=0 or variables not defined)",
        )
    ...
    if config_param.is_ConsiderBESS == 0
        println(
            "\t constraints: 11) stroges system constraints skipped (is_ConsiderBESS=0)",
        )
    else
        ...
        @constraint(
            ...
            [s = 1:NS, t = 1:NT],
            qc[((s-1)*NC+1):(s*NC), t] .==
            ((t == 1) ? Q_initial[:, 1] : qc[((s-1)*NC+1):(s*NC), t-1]) +
