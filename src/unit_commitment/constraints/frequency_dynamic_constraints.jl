...
function add_frequency_constraints!(scuc::Model, NT, NG, NC, NS, units, stroges, config_param, Δp_contingency; winds = nothing)
    ...
    thermal_inertia =
        @expression(scuc, [t = 1:NT], sum(safe_frequency_coeff(units.Hg, i) * safe_frequency_coeff(units.p_max, i) * x[i, t] for i in 1:NG))
    ...
    nadir_inertia_term = @expression(
        scuc,
        [t = 1:NT],
        (sum(safe_frequency_coeff(units.Hg, i) * safe_frequency_coeff(units.p_max, i) * x[i, t] for i in 1:NG) + wind_capacity_support.inertia) /
        sum_apparent_power
    )
    nadir_fg_response_term = @expression(
        scuc,
        [t = 1:NT],
        sum(unit_turbine_response_coeff(units, i) * safe_frequency_coeff(units.p_max, i) * x[i, t] for i in 1:NG) / sum_unit_capacity
    )
    nadir_primary_term = @expression(
        scuc,
        [t = 1:NT],
        sum(unit_primary_response_coeff(units, i) * safe_frequency_coeff(units.p_max, i) * x[i, t] for i in 1:NG) / sum_unit_capacity
    )

    rocof_constr = @constraint(
        scuc,
        [s = 1:NS, t = 1:NT],
    ...
    nadir_constr = @constraint(
        scuc,
        [k = 1:nadir_cut_count, s = 1:NS, t = 1:NT],
