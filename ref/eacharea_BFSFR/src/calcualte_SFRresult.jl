...
function form_SFRcurveData(signal, flag, whitenoise_parameter = 1e-4, fcr_selected_threshold = 100)
    if flag == 1
        if signal == 1
            ...
            δf_positor, δf_actual, δf_samplieddata =
                simulate(generate_data, particle_filter, 100, 60, 1, 1, 0, 1234, whitenoise_parameter, fcr_selected_threshold)
        ...
        elseif signal == 2
            ...
            # δf_ASFR, δf_different, δp_different = frequencydynamic_ASFR()
            δf_positor, δf_actual, δf_samplieddata =
                simulate(generate_data, particle_filter, 100, 60, 2, 1, 0, 1234, whitenoise_parameter, fcr_selected_threshold)
            # δf_ASFR, δf_different, δp_different = frequencydynamic_ASFR()
            # δf_ASFR = δf_positor
        ...
        if signal == 1
            ...
            # δf_ASFR, δf_different, δp_different = frequencydynamic_ASFR()
            δf_positor, δf_actual, δf_samplieddata =
                simulate(generate_data, particle_filter, 100, 60, 1, 1, 1, 1234, whitenoise_parameter, fcr_selected_threshold)
            # δf_ASFR = δf_positor
        ...
        elseif signal == 2
            ...
            # δf_ASFR, δf_different, δp_different = frequencydynamic_ASFR()
            δf_positor, δf_actual, δf_samplieddata =
                simulate(generate_data, particle_filter, 100, 60, 2, 1, 1, 1234, whitenoise_parameter, fcr_selected_threshold)
            # δf_ASFR, δf_different, δp_different = frequencydynamic_ASFR()
            # δf_ASFR = δf_positor
