...
base_path <- "d:/GithubClonefiles/RFCUC/RfcucCaseStudies/eacharea_BFSFR/res/Guassion/"
...
for (file_name in file_list) {
  ...
  p <- ggplot(data_long, aes(x = xdata, y = FrequencyDerivative, color = Model)) +
    ...
    scale_color_manual(values = c(
      ...
      "Simulator (reference frequency trajactories)" = "#565b59" # Cooked Asparagus Green
    )) +
  ...
  ggsave(output_filename, plot = p, width = 3.5, height = 3.5, units = "in", dpi = 300)
