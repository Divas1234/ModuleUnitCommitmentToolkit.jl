...
su_result_vector, sd_result_vector, op_result_vector, reserveplus_result_vector, reserveminus_result_vector =
    zeros(l1, l2), zeros(l1, l2), zeros(l1, l2), zeros(l1, l2), zeros(l1, l2)
for i in 1:l1
    for j in 1:l2
        try
            ~, ~, ~, ~, ~, ~, ~, ~, e_su_cost, e_sd_cost, e_prod_cost, e_cost_sr⁺, e_cost_sr⁻ = enhance_FCUC_scucmodel_withFCR(
                NN,
                NT,
                NB,
                NG,
...
CSV.write(
    "D:\\GithubClonefiles\\RFCUC\\RfcucCaseStudies\\littlecase\\output\\fcr\\res\\data_reserveplus_result_vector.csv",
    data_reserveplus_result_vector,
)
CSV.write(
    "D:\\GithubClonefiles\\RFCUC\\RfcucCaseStudies\\littlecase\\output\\fcr\\res\\data_reserveminus_result_vector.csv",
    data_reserveminus_result_vector,
)

e_x₀, e_p₀, e_pᵨ, e_pᵩ, e_seq_sr⁺, e_seq_sr⁻, e_pss_charge_p⁺, e_pss_charge_p⁻, e_su_cost, e_sd_cost, e_prod_cost, e_cost_sr⁺, e_cost_sr⁻ =
    enhance_FCUC_scucmodel_withFCR(
        NN,
        NT,
        NB,
        NG,
...
e_x₀, e_p₀, e_pᵨ, e_pᵩ, e_seq_sr⁺, e_seq_sr⁻, e_pss_charge_p⁺, e_pss_charge_p⁻, e_su_cost, e_sd_cost, e_prod_cost, e_cost_sr⁺, e_cost_sr⁻ =
    enhance_FCUC_scucmodel_withoutFCR(NN, NT, NB, NG, ND, NC, units, loads, winds, lines, config_param, fittingparameter_vector);
savebalance_result(e_p₀, e_pᵨ, e_pᵩ, e_pss_charge_p⁺, e_pss_charge_p⁻, 3);
txt_files_list = filter(f -> endswith(f, ".txt") && isfile(joinpath(obj_dir, f)), readdir(obj_dir; join = true))
