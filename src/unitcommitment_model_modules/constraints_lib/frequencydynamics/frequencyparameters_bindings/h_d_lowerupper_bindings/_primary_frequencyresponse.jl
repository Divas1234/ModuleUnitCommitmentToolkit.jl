...
# """
function inertia_bindings(damping, factorial_coefficient, time_content, droop, converter_vsm_parameters, converter_droop_parameters, flag)
	tem = zeros(length(damping), 2)
	for i in eachindex(damping)
		upper_bound_1, lower_bound_1 = inertia_damping_relations(damping[i], factorial_coefficient, time_content, droop, converter_vsm_parameters, converter_droop_parameters, flag)
...
# """
function inertia_damping_relations(damping::Float64, factorial_coefficient::Float64, time_content::Float64, droop::Float64, converter_vsm_parameters, converter_droop_parameters, flag::Int64)
	if flag == 0
		...
		tem1 = (damping + converter_vsm_parameters["damping"] + 1 / converter_droop_parameters["droop"] - factorial_coefficient + 2 * droop)
		tem2 = tem1^2 - (damping + converter_vsm_parameters["damping"] + 1 / converter_droop_parameters["droop"] + factorial_coefficient)^2
	end
...
"""
function calculate_inertia_parameters(initial_inertia::Float64, factorial_coefficient::Float64, time_constant::Float64, droop::Float64, power_deviation::Float64, damping_range, converter_vsm_parameters, converter_droop_parameters, flag_converter::Int64)
	...
	# zeta smaller than 1
	inertia_updown_bindings = view(inertia_bindings(damping_range, factorial_coefficient, time_constant, droop, converter_vsm_parameters, converter_droop_parameters, flag_converter), :, 1:2)
	...
	extreme_inertia, nadir_vector, inertia_vector, selected_ids = generate_extreme_inertia(
		initial_inertia, factorial_coefficient, time_constant, droop, power_deviation, damping_range, inertia_updown_bindings, converter_vsm_parameters, converter_droop_parameters, flag_converter::Int64
	)
...
"""
function estimate_inertia_limits(ROCOF_threshold::Float64, power_deviation::Float64, damping_range, factorial_coefficient::Float64, time_constant::Float64, droop::Float64)
