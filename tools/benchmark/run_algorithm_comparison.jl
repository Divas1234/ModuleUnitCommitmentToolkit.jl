...
function benders_data_tuple_to_namedtuple(
		config_param,
		units,
		lines,
		loads,
...
function run_benders_benchmark(scenario_count::Int64)
	...
	return capture_stdout_to_file(algorithm_log_path("benders", scenario_count), () -> begin
...
function run_ccg_benchmark(scenario_count::Int64)
	...
	return capture_stdout_to_file(algorithm_log_path("ccg", scenario_count), () -> begin
...
function run_extensive_form_benchmark(scenario_count::Int64)
	...
	return capture_stdout_to_file(algorithm_log_path("benchmark_uc", scenario_count), () -> begin
...
function write_iteration_history_csv(comparison_dir::AbstractString, entries)
	...
	open(path, "w") do io
		...
		for entry in entries
			...
			result = entry.result
			if algorithm_name == "benchmark_uc"
				println(io, join([algorithm_name, scenario_count, 1, scenario_count, result.lower_bound, result.upper_bound, result.gap, "", result_max_ram_mb(result)], ","))
				continue
			end
			for history_row in result.history
				...
				println(io, join([algorithm_name, scenario_count, history_row.iteration, active_scenarios, lower_bound, upper_bound, gap, added_scenarios, ram_mb], ","))
			end
		end
	end
	return path
...
function write_power_balance_quality_csv(comparison_dir::AbstractString, summary_rows)
	...
	open(path, "w") do io
		...
		for row in summary_rows
			...
			println(io, join([quality.algorithm, quality.scenarios, quality.max_abs_balance_error, quality.total_load_curtailment, quality.total_wind_curtailment, quality.peak_served_load], ","))
		end
	end
	return path
end
...
function write_benchmark_report(comparison_dir::AbstractString, run_id::AbstractString, scenario_counts::Vector{Int64}, summary_rows)
	...
	open(path, "w") do io
		...
		println(io, "|---|---:|---|---:|---:|---:|---:|---:|---:|")
		for row in summary_rows
			println(io, "| $(row.algorithm) | $(row.scenarios) | $(row.status) | $(row.iterations) | $(row.lower_bound) | $(row.upper_bound) | $(row.gap) | $(row.elapsed_seconds) | $(row.max_ram_mb) |")
		end
		println(io)
		println(io, "## Tables")
		println(io)
		...
		println(io, "- Benders evaluates scenario subproblems under first-stage commitments; the exported scheduling files use the best incumbent dispatch reconstruction.")
		println(io, "- CCG starts from a scenario subset and adds uncovered scenarios; its active-scenario history is recorded in `iteration_history.csv`.")
		println(io, "- Power-balance quality reports residual balance error, load curtailment, wind curtailment, and peak served load from each algorithm's generated power-balance CSV.")
	end
	return path
end
