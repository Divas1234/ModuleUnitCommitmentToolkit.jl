...
# Subproblem function (returns cuts)
function subproblem_callback(cb_data)
    ...
    if status == MOI.OPTIMAL
        ...
        cut = @expression(
            sub,
            sum(π[t] * demand[t] for t in 1:num_periods) +
...
MOI.set(master, MOI.UserCutCallback(), subproblem_callback)
MOI.set(master, MOI.LazyConstraintCallback(), subproblem_callback)
