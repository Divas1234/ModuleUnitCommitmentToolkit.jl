...
function write_experiment_summary(exp_dir::String, perf_path::String)
    ...
    out = DataFrame(
        Experiment = fill(label, nrow(df)),
        Scenario = fill(scenario, nrow(df)),
        Mode = df.Mode,
        TotalCost_USD = df.TotalCost_USD,
        CostGap_vs_All_pct = safe_col(df, :CostGap_vs_All_pct, 100.0 .* (df.TotalCost_USD .- baseline_cost) ./ baseline_cost),
        CostGap_vs_Best_pct = 100.0 .* (df.TotalCost_USD .- min_cost) ./ min_cost,
        SubproblemSolveTime_sec = df.SubproblemSolveTime_sec,
        ...
        JuliaAllocated_MB = df.JuliaAllocated_MB,
        MemoryGap_vs_Lowest_pct = 100.0 .* (df.JuliaAllocated_MB .- min_memory) ./ min_memory,
        AvgOverlap_h = df.AvgOverlap_h,
        MaxOverlap_h = df.MaxOverlap_h,
        MinOverlap_h = df.MinOverlap_h,
        LoadShedding_MWh = df.LoadShedding_MWh,
        WindCurtailment_MWh = df.WindCurtailment_MWh,
        LimitingFactorCounts = df.LimitingFactorCounts,
        CalibrationTime_excluded_sec = safe_col(df, :CalibrationTime_excluded_sec, missing),
        ReferenceTime_excluded_sec = safe_col(df, :ReferenceTime_excluded_sec, missing),
    )
    CSV.write(summary_csv, out)

    best_cost_row = out[argmin(out.TotalCost_USD), :]
    fastest_row = out[argmin(out.SubproblemSolveTime_sec), :]
    lowest_memory_row = out[argmin(out.JuliaAllocated_MB), :]
