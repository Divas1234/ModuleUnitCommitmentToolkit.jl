...
function fit_frequency_nadir_max_min_affine(X::Matrix{Float64}, y::Vector{Float64})
	...
	@constraint(fit_model, [i = 1:row_count, k = 1:cut_count],
		sum(coefficient[k, j] * X[i, j] for j in 1:4) <= y[i])
	@constraint(fit_model, [i = 1:row_count, k = 1:cut_count],
	...
	@objective(fit_model, Min,
		1.0e3 * max_error +
		sum(fitting_error[i] for i in 1:row_count) +
		regularization * sum(coefficient_abs[k, j] for k in 1:cut_count, j in 1:4))
