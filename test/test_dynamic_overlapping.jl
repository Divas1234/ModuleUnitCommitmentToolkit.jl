...
# ============================================================================
function solve_rolling_uc(
    net_load_profile::Vector{Float64},
    units::Vector{Unit},
    T_D::Int,
    T_lookahead_max::Int;
    delta_ramp::Float64 = 40.0,
    T_o_min::Int = 1,
    T_o_max::Int = 12,
    enable_dynamic_overlapping::Bool = true,
    enable_space_decoupling::Bool = true
)
    total_hours = length(net_load_profile)
    current_start = 1
    
    # Initialize previous state tracking
    pre_states = Dict(g.id => (x0=g.initial_x, t_elapsed=g.initial_t) for g in units)
    ...
    while current_start <= total_hours - T_D
        ...
        for g_idx in 1:NG
            ...
            if state_info.x0 == 1
                ...
                for t in 1:min(L_up, actual_T_sub)
                ...
                for t in 1:min(L_down, actual_T_sub)
        ...
        for t in 1:actual_T_sub
            @constraint(model, sum(p[g_idx, t] for g_idx in 1:NG) + slack_up[t] - slack_dn[t] == sub_net_load[t])
        end
        ...
        for t in 1:T_D
            ...
            p_val = [value(p[g_idx, t]) for g_idx in 1:NG]
            x_val = [round(Int, value(x[g_idx, t])) for g_idx in 1:NG]
        ...
        for g_idx in 1:NG
            ...
            if x_end == prev_info.x0
                ...
                history_states = [round(Int, value(x[g_idx, t])) for t in 1:T_D]
...
function run_debugging_demo()
    ...
    units = [
        Unit(1, "Slow_Coal_A",  300.0,  100.0,  15.0,  3000.0,  12,  12,  1,  10,  true),
        Unit(2, "Slow_Coal_B",  250.0,   80.0,  17.0,  2500.0,  10,  10,  1,   5,  true),
        Unit(3, "Fast_Gas_C",   120.0,   30.0,  30.0,   500.0,   2,   2,  0,   4,  false),
        Unit(4, "Fast_Gas_D",   100.0,   20.0,  35.0,   400.0,   1,   1,  0,   6,  false)
    ]
    ...
    res_dynamic = solve_rolling_uc(
        net_load_profile, units, T_D, T_lookahead_max,
        delta_ramp = 35.0, T_o_min = 1, T_o_max = 12,
        enable_dynamic_overlapping = true, enable_space_decoupling = true
    )
    
    # 2. Run Baseline Model: Fixed 4-hour Overlapping + No Space Decoupling
    res_baseline = solve_rolling_uc(
        net_load_profile, units, T_D, T_lookahead_max,
        enable_dynamic_overlapping = false, enable_space_decoupling = false
    )
    ...
    @printf("  %-30s | %18.2f%% | %18s\n", "Solve Time Saved (%)", 
        (res_baseline["solve_time"] - res_dynamic["solve_time"]) / res_baseline["solve_time"] * 100, "Ref. 0.00%")
    @printf("  %-30s | %18.3f%% | %18s\n", "Cost Deviation (%)", 
        (res_dynamic["cost"] - res_baseline["cost"]) / res_baseline["cost"] * 100, "Ref. 0.00%")
    println("-"^80)
    ...
    println("="^80)
    for g_idx in 1:length(units)
        @printf("  Unit %d (%-12s): ", g_idx, units[g_idx].name)
        for t in 1:12:144 # Sample every 12 hours for output clarity
