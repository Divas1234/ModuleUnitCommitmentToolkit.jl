...
function frequencydynamic_pres(Mg, Hg, Dg, Tg, Rg, Fg, Kg, Rw, Dw, Mw, Hw, Tw, Kw, δp, D, H, T, endtime)
    ...
    for i in 1:size(xdata, 1)
        ...
        a₁, a₂, a₃, T = param[1, 1] * 1, param[2, 1] * 0.5, param[3, 1] * 0.5, param[4, 1] * 2

        λ = sqrt(Complex(a₂ * a₂ - 4 * a₁ * a₃))
        α = exp(0.0 - (a₂ + λ) / (2 * a₁) * t)
        β = 0.0 - λ - λ * exp(λ / a₁ * t) + λ * 2 * exp((a₂ + λ) / (2 * a₁) * t) - (0.0 - 1 + exp(λ / a₁ * t)) * (a₂ - 2 * a₃ * T)
        γ = (2 * a₃ * λ)
        δf = real(α * β / γ) * 1.0
        ydata[i, 1] = f_base - δf
    end
