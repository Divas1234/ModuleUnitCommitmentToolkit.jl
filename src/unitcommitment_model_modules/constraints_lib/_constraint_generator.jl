...
function estimate_initial_status(
    units,
    NG,
    NT,
    onoffinit,
...
function add_unit_operation_constraints!(scuc::Model, NT, NG, units, onoffinit)
    ...
    units_init_stateslogic_consist_constr = @constraint(
        scuc,
        [i = 1:NG, t = 1:NT],
...
function add_pwl_constraints!(scuc::Model, NT, NG, NS, units)
    ...
    units_pwlpower_sum_constr = @constraint(
        ...
        pg₀[i+(s-1)*NG, t] .==
        p_min[i, 1] * x[i, t] + sum(pgₖ[i+(s-1)*NG, t, k] for k ∈ 1:num_segments)
    )
    units_pwlblock_upbound_constr = @constraint(
        scuc,
...
function add_hydros_constraints!(scuc::Model, NT, NH, NS, hydros)
    ...
    hydros_minpower_constr =
        @constraint(scuc, [s = 1:NS, t = 1:NT], ph[((s-1)*NH+1):(s*NH), t] .>= p_min[:, 1])
    hydros_maxpower_constr =
        @constraint(scuc, [s = 1:NS, t = 1:NT], ph[((s-1)*NH+1):(s*NH), t] .<= p_max[:, 1])
    hydros_resvoir_constr = @constraint(
        scuc,
        [s = 1:NS, t = 1:NT],
        ph[((s-1)*NH+1):(s*NH), t] .<= cumsum_resvoir[t, 1]
    )

    hydros_cumsum_reservoir_constr = @constraint(
        scuc,
        [s = 1:NS],
        q0[:, 1] .+ sum(
            ones(NH, 1) * cumsum_resvoir[t, 1] .- ph[((s-1)*NH+1):(s*NH), t] for t ∈ 1:NT
        ) .<= qmax[:, 1]
    )
    ...
    hydros_minpower_constr,
    hydros_maxpower_constr,
    hydros_resvoir_constr,
    hydros_cumsum_reservoir_constr
end
