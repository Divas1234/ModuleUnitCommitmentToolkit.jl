...
function add_datacentra_constraints!(
    scuc::Model,
    NT,
    NS,
    config_param,
    ...
    if config_param.is_ConsiderDataCentra == 0
        println(
            "\t constraints: 12) data centra constraints skipped (is_ConsiderDataCentra != 1)",
        )
    else
        dc_p = scuc[:dc_p]
        dc_f = scuc[:dc_f]
        ...
        @constraint(
            ...
            idale_dc .+ sv_const_dc .* dc_Δu2[((s-1)*ND2+1):(s*ND2), t] ./ mu_dc
        )
        ...
        @constraint(
            ...
            dc_v²[((s-1)*ND2+1):(s*ND2), t] .+ dc_f[((s-1)*ND2+1):(s*ND2), t] .-
            ones(ND2, 1)
        ) # Assuming v², f are binary [0,1]
        ...
        @constraint(
            ...
            dc_λ[((s-1)*ND2+1):(s*ND2), t] .+ dc_Δu1[((s-1)*ND2+1):(s*ND2), t] .-
            ones(ND2, 1)
        ) # Assuming Δu1, λ are binary [0,1]
        ...
        @constraint(
            scuc,
            [s = 1:NS, t = 1:NT],
            dc_v²[((s-1)*ND2+1):(s*ND2), t] .<= ones(ND2, 1)
        ) # Assuming v² represents a binary/normalized value
        ...
        if comp_tasks !== nothing && length(comp_tasks) == NT
            @constraint(
                ...
                sum(
                    dc_λ[((s-1)*ND2+1):(s*ND2), ((iter-1)*iter_block+1):(iter*iter_block)],
                ) .<=
                (1 + coeff) * sum(DataCentras.λ) * iter_block .* sum(
                    DataCentras.computational_power_tasks[((iter-1)*iter_block+1):(iter*iter_block)],
                )
            )
            @constraint(
                scuc,
            ...
            println(
                "Warning: DataCentras.computational_power_tasks or DataCentras.λ not suitable for constraints. Skipping related constraints.",
            )
        end
